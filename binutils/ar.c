  ...
  for (; count > 0; files++, count--)
      ...
      for (head = arch->next; head; head = head->next)
	{
	  ...
	  if (head->filename != NULL) 
		...
		{
		  match_count++;
		  if (match_count == counted_name_counter) 
		    {
		      /* if this matched, then we must be doing a counted
			 extract!  We must bail out after doing it. */
		      function (head, *files);
		      exit(0);
		    }
		  found = true;
     ...
     char **argv;
{
  ...
  if (mri_mode)
      ...
      if (counted_name_mode) 
	{
	     ...
	     fatal("`N' is only meaningful with the `x' option.");
     ...
     char *target_filename;
{
  ...
  if (bfd_stat_arch_elt (abfd, &buf) != 0)
    fatal ("internal stat error on %s", target_filename);
  size = buf.st_size;
  ...
  if (ar_truncate)
    {
      /* This should really use bfd_set_file_flags, but that rejects
         archives.  */
      obfd->file_flags |= BFD_TRADITIONAL_FORMAT;
    }
