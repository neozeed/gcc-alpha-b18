...
#include <stdio.h>              /* zzapp */
#include <string.h>             /* zzapp */
#include <sys/types.h>          /* zzapp */
#include <sys/stat.h>           /* zzapp */
#include <fcntl.h>              /* zzapp */

#if 0
#include <unistd.h>             /* zzapp */
#endif

#include <time.h>               /* zzapp */
...
typedef struct ifunct      /* zzapp */
  {
    char          *name;   /* name of function being imported */
    short          ord;    /* ordinal value associated with function */
    struct ifunct *next;
  } 
ifunctype;

typedef struct iheadt      /* zzapp */
  {
...
/* ignore_imports: if nonzero, IMPORT declarations are ignored   */
/*   and no .import section will be created.  Initialized in     */
/*   function main() to IGNOREIMPORTS.                           */
...
/*   not defined until function calculate_import_size() (qv) has */
/*   run.  Used also in function write_imports().                */
/* ttk_kludge: cheap trick to determine endianness of the object */
/*   file; the object file is opened as a bfd and ttk_kludge is  */
/*   pointed at the descriptor.  Functions LONG() and SHORT()    */
/*   are used in flipheader(), flipsection(), and flipidata()    */
/*   functions to reverse the header, section, and idata data    */
/*   before they are written out to the object file.  LONG() and */
/*   SHORT() refer to ttk_kludge to determine if the reversal is */
/*   actually necessary.  ttk_kludge is a global because I did   */
/*   not like having to always pass an extra parameter to LONG() */
/*   and SHORT().                                                */

char         ignore_imports;  /* zzapp */
iheadtype   *import_list;     /* zzapp */
long         nheads;          /* zzapp */
bfd         *ttk_kludge;      /* zzapp */ /* apologies in advance */

#define  IGNOREIMPORTS  1     /* zzapp */
...
#ifdef DLLTOOL_ALPHA
static char *mname = "alpha";
#endif

#define PATHMAX 250             /* What's the right name for this ? */

/*  ttk@cygnus.com, 1997  */
/*  Since I haven't been at Cygnus very long, I'm still learning   */
/*  how to use the bfd library.  Different parts of my code were   */
/*  written with either bfd in mind or just writing binary codes   */
/*  right into the object file.  I wound up ditching my bfd code.  */
/*  If the PE format changes, I will take my lumps and rewrite it  */
/*  the "right way" (using bfd).  With that preamble in mind:      */

/*    The PE object file consists of a header, a bunch of   */
/*  section headers, a symbol table, and the section data.  */
/*  In function ttk_gather() the object header is read into */
/*  a headertype structure, the sections are each read into */
/*  different sectiontype structures, and everything else   */
/*  is read into a dynamic buffer.  Later, in ttk_emit(),   */
/*  some of the values in the header and section headers    */
/*  are diddled with, an idata section is added, and every- */
/*  thing gets written back out to the object file.  The    */
/*  actual idata section data is appended to the end of the */
/*  file.                                                   */

typedef struct headertag        /* zzapp */
  {
    short machine;
    short nsections;
    long  timedate;
...
typedef struct sectiontag       /* zzapp */
  {
    char name[8];
    long reladdr;
    long vaddr;       /* useless in OBJ */
    long datasize;    /* exact size of section */
    long dataaddr;    /* file-based offset of section */
    long dontcare[3];
    long flags;
  } 
sectiontype;

/*    The idatatype structure is an exact representation of */
/*  the first part of the idata section structure.  .idata  */
/*  begins with one idatatype structure for every dll file  */
/*  from which functions are imported.  It is terminated by */
/*  an idatatype structure containing all 0 values.  */

typedef struct idatatag         /* zzapp */
  {
    long listaddr1;
    long timedate;
    long unusedchain;
...
headertype    header;           /* zzapp */
sectiontype   section[32];      /* zzapp */
idatatype     idata[256];       /* zzapp */
int           curpos;           /* zzapp */
...
LONG ( n )
  long n;
{ long retval; 
  bfd_put_32 ( ttk_kludge, n, (char*)(&retval) );
  return retval;
}
...
SHORT ( n ) 
  short n; 
{ short retval;
  bfd_put_16 ( ttk_kludge, n, (char*)(&retval) );
  return retval;
}

flipheader() /*zzapp */
{
  header.machine = SHORT(header.machine);
  header.nsections = SHORT(header.nsections);
  header.timedate = LONG(header.timedate);
  header.tosymboltable = LONG(header.tosymboltable);
  header.nsymbols = LONG(header.nsymbols);
  header.optheadersize = SHORT(header.optheadersize);
  header.flags = SHORT(header.flags);
}

flipsection ( i ) /* zzapp */
  int i; 
{ 
  section[i].reladdr  = LONG ( section[i].reladdr );
  section[i].vaddr    = LONG ( section[i].vaddr   );
  section[i].datasize = LONG ( section[i].datasize);
  section[i].dataaddr = LONG ( section[i].dataaddr);
  section[i].dontcare[0] = LONG ( section[i].dontcare[0] );
  section[i].dontcare[1] = LONG ( section[i].dontcare[1] );
  section[i].dontcare[2] = LONG ( section[i].dontcare[2] );
  section[i].flags    = LONG ( section[i].flags   );
}

flipidata ( i )  /* zzapp */
  int i; 
{ 
  idata[i].listaddr1   = LONG ( idata[i].listaddr1   );
  idata[i].timedate    = LONG ( idata[i].timedate    );
  idata[i].unusedchain = LONG ( idata[i].unusedchain );
  idata[i].nameaddr    = LONG ( idata[i].nameaddr    );
  idata[i].listaddr2   = LONG ( idata[i].listaddr2   );
} 
  ...
  char *infn, *oufn; 
{
  ...
  flipheader();
     for ( i = 0; i < header.nsections; i++ ) 
       { 
	 fread ( &(section[i]), sizeof ( sectiontype ), 1, fin );
	 flipsection(i);
	 if ( !strncmp ( section[i].name, ".edata", 6 ) ) 
	    e_index = i;
       } /* END of for */
  platz   = ftell ( fin );
  fseek  ( fin, 0, SEEK_END );
  eofhere = ftell ( fin );
  fseek  ( fin, platz, SEEK_SET );
  smag = eofhere - platz;
  stuff = (char*) malloc ( smag );
  fread  ( stuff, smag, 1, fin );
  fclose ( fin );
} /* END of function ttk_gather */

ttk_emit ( idatas, imag, iflag )  /* zzapp */
  char *idatas; 
  long imag, iflag; 
{
  int i, nsec;

  nsec = header.nsections;
  header.nsections++;
  if ( header.tosymboltable ) 
     header.tosymboltable += sizeof ( sectiontype );
  flipheader();
  fwrite ( &header, sizeof ( header ), 1, fou );
  for ( i = 0; i < nsec; i++ ) 
    if ( i != e_index ) 
      {
	flipsection(i);
	fwrite ( &(section[i]), sizeof ( sectiontype ), 1, fou );
      } else {
	section[i].dataaddr += sizeof ( sectiontype );
	flipsection(i);
	fwrite ( &(section[i]), sizeof ( sectiontype ), 1, fou );
	strcpy ( section[i].name, ".idata" );
	section[i].name[6]  = 0;
	section[i].name[7]  = 0;
	section[i].reladdr  = 0;
	section[i].vaddr    = 0;
	section[i].datasize = imag;
	section[i].dataaddr = eofhere + sizeof ( sectiontype );
	section[i].dontcare[0] = 0;
	section[i].dontcare[1] = 0;
	section[i].dontcare[2] = 0;
	section[i].flags    = iflag;
	flipsection(i);
	fwrite ( &(section[i]), sizeof ( sectiontype ), 1, fou );
      } /* END of for loop and if..else clause */
  fwrite ( stuff,  smag, 1, fou );
  fwrite ( idatas, imag, 1, fou );
  fclose ( fou );
} /* END of function ttk_emit */
...
unsigned char alpha_jtab[] = 
{
	0x00,0x00,0x7f,0x27,  /* ldl  $t12,__imp_xxx      */
	0x00,0x00,0x7b,0xa3,
	0x00,0x00,0xfb,0x6b   /* jmp ($t12)               */
};
...
#define MPPC 2
  ...
  {
#define ALPHA 3
   "alpha" ,".byte", ".short", ".long", ".asciz", "#", "jmp ", ".global", ".space", ".align\t2",".align\t4","pe-alpha",bfd_arch_alpha,
  alpha_jtab,sizeof(alpha_jtab),0,
  }
  ,
{    0}
};
...
{ 
  ...
  return;
  #endif
  fprintf (stderr,"TTKDEBUG %s: %s\n", program_name, s );
} /* END of function ttkdebug */
...
{ iheadtype *finger;
  ...
  if ( import_list == NULL ) {
    ...
    import_list->dllname  = strdup ( dll_name );
    ...
    import_list->functail->name = strdup ( symbol_name );
    import_list->functail->ord  = atoi ( func_ordinal );
    import_list->functail->next = NULL;
    return;
  } /* END of case import_list == NULL */
  finger = import_list;
  while ( ( strcmp ( finger->dllname, dll_name ) ) &&
	  ( finger->next != NULL                 )    )
    finger = finger->next; 
  if ( !strcmp ( finger->dllname, dll_name ) ) {
    finger->functail->next = xmalloc ( sizeof ( ifunctype ) );
    finger->functail = finger->functail->next;
    finger->functail->ord  = atoi ( func_ordinal );
    finger->functail->name = strdup ( symbol_name );
    finger->functail->next = NULL;
    finger->nfuncs++;
  } else { /* this dll doesn't already have entry */
    finger->next = xmalloc ( sizeof ( iheadtype ) );
    finger = finger->next;
    finger->dllname  = strdup ( dll_name );
    finger->nfuncs   = 1;
    finger->funchead = xmalloc ( sizeof ( ifunctype ) );
    finger->functail = finger->funchead;
    finger->next     = NULL;
    finger->functail->name = strdup ( symbol_name );
    finger->functail->ord  = atoi ( func_ordinal );
    finger->functail->next = NULL;
...
void 
def_import (mode, internal, module, entry, ord_val)
     int   mode;
     char *internal;
     char *module;
     char *entry;
...
{ char *internal_name;
  ...
  switch ( mode ) 
    {
    case 0 : internal_name = entry;
	     module_name   = (char*) malloc(strlen(module)+4);
	     strcpy ( module_name, module );
	     strcat ( module_name, ".dll" );
	     entry_name    = "0"; 
	     gotalloc      = 1;
	     break;
    case 1 : internal_name = internal;
	     module_name   = (char*) malloc(strlen(module)+4);
	     strcpy ( module_name, module );
	     strcat ( module_name, ".dll" );
	     ...
	     gotalloc      = 1;
	     break;
    case 2 : internal_name = entry;
	     module_name   = module;
	     entry_name    = "0";
	     break;
    case 3 : internal_name = internal;
	     module_name   = module;
	     entry_name    = ord_string;
	     break;
    case 4 : internal_name = entry;
	     module_name   = (char*) malloc(strlen(module)+4);
	     strcpy ( module_name, module );
	     strcat ( module_name, ".dll" );
	     entry_name    = "0"; 
    ...
    case 5 : internal_name = "";
	     ...
	     gotalloc      = 1;
	     break;
    case 6 : internal_name = entry;
	     module_name   = module;
	     entry_name    = "0";
	     break;
    case 7 : internal_name = "";
	     module_name   = module;
	     entry_name    = ord_string;
	     break;
    } /* END of switchbox */
  append_import ( internal_name, module_name, entry_name ); 
  if ( gotalloc ) free ( module_name );
...
#define _P_WAIT         0
  {
	  int spawnresult;
	  spawnresult = _spawnvp( _P_WAIT,what,argv);
	  if (spawnresult == -1)
	  {
		  fprintf (stderr, "%s: can't exec %s\n", program_name, what);
		  exit (1);
	  ...
	  {
		  fprintf (stderr, "%s: %s %s terminated with exit status %d\n",
		       program_name, what, args, spawnresult);
	      exit (1);	
	  }
  }
#endif
...
{  
  ...
  nheads = 0;
  for ( fingerh = import_list; fingerh != NULL; fingerh = fingerh->next ) 
    {
      nheads++;
      retval += 20;  /* size of idata header record */
      retval += strlen ( fingerh->dllname ) + 1;
      for ( fingerf  = fingerh->funchead; fingerf != NULL; fingerf = fingerf->next ) 
	{ 
	   retval += 10; /* two pointers and short for ord */
	   retval += strlen ( fingerf->name ) + 1;
	} /* END of for ( fingerf ) loop */
    retval += 8; /* two null pointers for terminus */
  } /* END of for ( fingerh ) loop */
  retval += 20; /* null idata header record */
  return retval;
} /* END of function calculate_import_size() */

typedef struct idata_real_tag  /* zzapp */
  {  
    long rva_list1;
    long zero1;
    long zero2;
...
void /* zzapp */
append_old_imports ( d, base )
  bfd_byte *d;    /* section contents */
  bfd_vma   base; /* base address of section */
{ idata_realt hand;
  short       ord;
  char        ordstr[8];
  char        fname[1024];
  ...
  memcpy ( &hand, d, sizeof ( hand ) ); 
  while ( hand.rva_list1 ) 
    {
      hand.rva_list1   = LONG ( hand.rva_list1   );
      hand.rva_dllname = LONG ( hand.rva_dllname );
      strcpy ( dname,   d + hand.rva_dllname - base  );
      memcpy ( &finger, d + hand.rva_list1 - base, 4 );
      finger = LONG ( finger );
      while ( finger ) 
	{
	  memcpy ( &ord,  d + finger - base,  2 );
	  strcpy ( fname, d + finger - base + 2 );
	  sprintf( ordstr, "%u", ord );
	  append_import ( fname, dname, ordstr );
	  hand.rva_list1 += 4;
	  memcpy ( &finger, d + hand.rva_list1 - base, 4 );
	  finger = LONG ( finger );
	} /* END of while */
      index += sizeof ( hand );
      nheads++;
      memcpy ( &hand, d + index, sizeof ( hand ) );
    } /* END of while */
} /* END of function append_old_imports() */

void /* zzapp */
write_imports ( data, bvma )
  bfd_byte *data;
  bfd_vma   bvma;
{ int          index, index0;
  int          l1pos, l2pos, targ, i;
  short        ord;
  idata_realt  hand;
  iheadtype   *finger;
  ifunctype   *thumb;
  ...
  hand.zero1 = 0;
  hand.zero2 = 0;
  for ( finger = import_list; finger != NULL; finger = finger->next ) 
    {
      /* write the dll name */
      hand.rva_dllname = index + bvma;
      strcpy ( data + index, finger->dllname );
      index += strlen ( finger->dllname ) + 1;
      /* build the lists */
      l1pos = index;
      hand.rva_list1 = index + bvma;
      index += ( finger->nfuncs + 1 ) * 4;
      l2pos = index;
      hand.rva_list2 = index + bvma;
      index += ( finger->nfuncs + 1 ) * 4;
      /* traverse + fill the lists */
      thumb = finger->funchead;
      for ( i = 0; i < finger->nfuncs; i++ ) 
	{
	  targ = LONG(index + bvma);
	  memcpy ( data + l1pos + (4*i), &targ, 4 );
	  memcpy ( data + l2pos + (4*i), &targ, 4 );
	  ord = SHORT(thumb->ord);
	  memcpy ( data + index, &ord, 2 );
	  index += 2;
	  strcpy ( data + index, thumb->name );
	  index += strlen ( thumb->name ) + 1;
	  thumb = thumb->next;
	} /* END of for loop */
      targ = 0;
      memcpy ( data + l1pos + (4*i), &targ, 4 );
      memcpy ( data + l2pos + (4*i), &targ, 4 );
      /* write out the contents of hand to data + index0 */
      hand.rva_list1   = LONG(hand.rva_list1);
      hand.rva_list2   = LONG(hand.rva_list2);
      hand.rva_dllname = LONG(hand.rva_dllname);
      memcpy ( data + index0, &hand, sizeof ( hand ) );
      index0 += sizeof ( hand );
    } /* END of for loop */
  hand.rva_list1   = 0;
  hand.rva_list2   = 0;
  hand.rva_dllname = 0;
  memcpy ( data + index0, &hand, sizeof ( hand ) );
} /* END of function write_imports() */

void
add_idata_sections ( oname )  /* zzapp */
  char *oname;                                       
{ bfd      *abfd;                                    
  int       finger = 0;                              
  int       isize;
  ...
  tname = (char*) xmalloc ( strlen(oname) + 2 ); 
  strcpy ( tname, oname );
  strcat ( tname, "t" );
  bfd_init();                                    
  abfd = bfd_openr ( oname, NULL );              
  ttk_kludge = abfd;
  isize = calculate_import_size();               
  data  = xmalloc ( isize );                     
  write_imports ( data, 0 );                     
  ttk_gather ( oname, tname ); 
  ttk_emit   ( data, isize, 0 );
  free ( data );                                 
  bfd_close ( abfd );                            
  sprintf ( command_string, "cp %s %s", tname, oname );
  system  ( command_string );
  sprintf ( command_string, "rm %s", tname );
  system  ( command_string );
} /* END of function add_idata_sections */       


void
gen_exp_file ()
     /* note: this is a misnomer; taking care of imports here too */
     /* well, qv add_idata_sections(), called at end of this func */
{
  FILE *f;
  int i;
  export_type *exp;
...
/*        fprintf (f, "\t%s\t0,0\t%s End\n", ASM_LONG, ASM_C);*/
  ...
  if ( import_list != NULL )           /* zzapp */
     add_idata_sections ( exp_name );  /* zzapp */
     ...
     int i;
{
  ...
  if (0)
#endif
    {
      ...
      fprintf (f, "\t%s\t__imp_%s\n", ASM_GLOBAL, exp->name);
#ifndef DLLTOOL_ALPHA
      fprintf (f, "%s%s:\n\t%s\t__imp_%s\n", ASM_PREFIX,
	       exp->name, ASM_JUMP, exp->name);
#else
	  fprintf (f, "%s%s:\n\t%s\t$1,__imp_%s\n\tldl\t$27,0($1)\n\tjmp\t($27)\n", ASM_PREFIX,
			exp->name,"lda",exp->name);
#endif
     ...
     int status;
{
  ...
  fprintf (file, "   -I                     Don't ignore imports\n"); /* zzapp */
     ...
     char **av;
{
  ...
  program_name = av[0];
  oav = av;
  ignore_imports = IGNOREIMPORTS;  /* zzapp */
  import_list = NULL;              /* zzapp */

  while ((c = getopt_long (ac,av,"Ixcz:S:R:A:puaD:l:e:nkvbUh?m:yd:",long_options,0)) != EOF) /* zzapp */
    {
      switch (c)
	{
	...
	case 'l':
	  imp_name = optarg;
	  break;
	case 'e':  /* fnord; .o file to stick sections into */
	  exp_name = optarg;
	  break;
	case 'h':
	case '?':
  ...
  if (!dll_name && exp_name)
    {
      int len = strlen (exp_name) + 5; /* zzapp */
  ...
  if (def_file)
    {
      process_def_file (def_file); /* fnord */
    }
  while (optind < ac)
    {
      if (!firstarg)
	firstarg = av[optind];
      scan_obj_file (av[optind]);  /* alpha */
      optind++;
    }
