  ...
  if (num <= 0) 
    {
	 ...
	 havoc later */
      as_fatal("Line numbers must be positive integers\n");
    }
     ...
     int ignore;
{
  ...
  if (def_symbol_in_progress != NULL)
    {
      as_warn (".loc pseudo-op inside .def/.endef: ignored.");
      demand_empty_rest_of_line ();
      return;
    }
     ...
     int ignore;
{
  ...
  symbol_name = tc_canonicalize_symbol_name (symbol_name);
      ...
      else if (strcmp (S_GET_NAME (def_symbol_in_progress), symbol_name))
	{
      ...
      *input_line_pointer = name_end;
      /* the NON-BFD case flushes the rest of the line here;
	 so far, at least, it doesn't seem to be required. */
    }
  else
    {
...
void coff_frob_file PARAMS((void));
void
coff_frob_file (void)
{
...
#endif
#if defined(TC_ALPHA)
                      /* The Alpha (Under NT) has a similar kluge for
			 REFHI/PAIR.  We've already calculated the offset
			 in the next relocation, but we don't want to
			 process the "symbol" field as a symbol.  
			 Assumption: that PAIR always immediately follows 
			 REFHI, which it always should.  (Not true, however
			 for REFLO, which could end up separated.) */

		      if (intr.r_type == IMAGE_REL_ALPHA_REFHI) 
			{
		  	  fix_ptr = fix_ptr->fx_next;
		          intr.r_type = IMAGE_REL_ALPHA_PAIR;
			  intr.r_symndx = fix_ptr->fx_addnumber;
		          /* intr.r_vaddr doesn't change */
			  (void) bfd_coff_swap_reloc_out (abfd, &intr, ext_ptr);
			  ext_ptr++;
			}
#endif
...
/*
 * Handle .loc directives, just for compatability
 */
