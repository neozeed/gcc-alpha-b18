...
#ifndef TE_PE  /* we're always 32-bit addressed */
...
alpha_fix_adjustable (f)
     fixS *f;
{
/* This seems useful in the MS COFF case as well, so until we find a contrary
...
#define EVAX_SECTION_COUNT 6
  ...
  static char *section_name[EVAX_SECTION_COUNT+1] =
    { "NULL", ".rdata", "<error>", "<error>", ".ctors", ".dtors", "<error>" };
#else
  static char *section_name[EVAX_SECTION_COUNT+1] =
...
#ifdef TE_PE
  /* the next flag would be gp mode usage, but since we don't use it... 
     ...
     int size;
{
  ...
  if (alpha_auto_align_on && alpha_current_align < log_size)
    alpha_align (log_size, (char *) NULL, NULL);
  if (alpha_current_align > log_size)
    alpha_current_align = log_size;
  alpha_insn_label = NULL;
}
