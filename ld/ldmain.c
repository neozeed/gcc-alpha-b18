...
/* we need to keep track of the order in which we actually add members;
   see the sort routines in emultempl/pe.em; this can be across all 
   archives, so don't bother resetting between archives, as it isn't
   a counter, just an ordinal */
static int encounter_position = 0;
     ...
     const char *name;
{
  ...
  input->encounter_position = encounter_position++;
