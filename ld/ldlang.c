...
#else
    p = (  lang_input_statement_type  *) new_statement(  lang_input_statement_enum  , sizeof(  lang_input_statement_type  ),  stat_ptr ) ;
#endif
  else
    {
...
#else
	  lookup = (lang_output_section_statement_type *)
	(  lang_output_section_statement_type  *) new_statement(  lang_output_section_statement_enum  , sizeof(  lang_output_section_statement_type  ),  stat_ptr ) ;
#endif
	  ...
	  strcmp (sec->comdat_info.name, l->sec->comdat_info.name) == 0)
	{
	  ...
	  switch (flags & SEC_LINK_DUPLICATES)
	    ...
	    case SEC_LINK_DUPLICATES_ONE_ONLY:
	      einfo ("%P: %B: warning: ignoring duplicate `%s' section symbol `%s'\n",
...
#else
	  new = (  lang_input_section_type  *) new_statement(  lang_input_section_enum  , sizeof(  lang_input_section_type  ),  ptr ) ;
#endif
      new->section = section;
      new->ifile = file;
     ...
     lang_statement_union_type * statement;
{
  switch (statement->header.type)
	...
	output_bfd->file_flags |= D_PAGED;
      else
	output_bfd->file_flags &= ~D_PAGED;
      if (config.text_read_only)
	output_bfd->file_flags |= WP_TEXT;
      else
	output_bfd->file_flags &= ~WP_TEXT;
      if (link_info.traditional_format)
	output_bfd->file_flags |= BFD_TRADITIONAL_FORMAT;
      else
	output_bfd->file_flags &= ~BFD_TRADITIONAL_FORMAT;
...
#else
  lang_output_statement_type *new =
    (  lang_output_statement_type  *) new_statement(  lang_output_statement_enum  , sizeof(  lang_output_statement_type  ),  stat_ptr ) ;
#endif
     ...
     CONST char *CONST filename;
{
  ...
  lang_wild_statement_type *new = new_stat (lang_wild_statement,
					    stat_ptr);
#else
  lang_wild_statement_type *new = (  lang_wild_statement_type  *) new_statement(  lang_wild_statement_enum  , sizeof(  lang_wild_statement_type  ), 
					    stat_ptr ) ;
#endif
...
#else
  lang_address_statement_type *ad = (  lang_address_statement_type  *) new_statement(  lang_address_statement_enum  , sizeof(  lang_address_statement_type  ),  stat_ptr ) ;
#endif
     ...
     CONST char *name;
{
  ...
  lang_target_statement_type *new = new_stat (lang_target_statement,
					      stat_ptr);
#else
  lang_target_statement_type *new = (  lang_target_statement_type  *) new_statement(  lang_target_statement_enum  , sizeof(  lang_target_statement_type  ), 
					      stat_ptr ) ;
#endif
  new->target = name;
     ...
     int exp;
{
  ...
  lang_fill_statement_type *new = new_stat (lang_fill_statement,
					    stat_ptr);
#else
  lang_fill_statement_type *new = (  lang_fill_statement_type  *) new_statement(  lang_fill_statement_enum  , sizeof(  lang_fill_statement_type  ), 
					    stat_ptr ) ;
#endif
  new->fill = exp;
}
     ...
     union etree_union *exp;
{
  ...
  lang_data_statement_type *new = new_stat (lang_data_statement,
					    stat_ptr);
#else
  lang_data_statement_type *new = (  lang_data_statement_type  *) new_statement(  lang_data_statement_enum  , sizeof(  lang_data_statement_type  ), 
					    stat_ptr ) ;
#endif
  new->exp = exp;
  new->type = type;
     ...
     etree_type * exp;
{
  ...
  lang_assignment_statement_type *new = new_stat (lang_assignment_statement,
						  stat_ptr);
#else
  lang_assignment_statement_type *new = (  lang_assignment_statement_type  *) new_statement(  lang_assignment_statement_enum  , sizeof(  lang_assignment_statement_type  ), 
						  stat_ptr ) ;
#endif
  new->exp = exp;
}
     ...
     const char *name;
{
  ...
  if (overlay_max == NULL)
    ...
    overlay_max = exp_binop (MAX_FUNC, overlay_max, size);
}
