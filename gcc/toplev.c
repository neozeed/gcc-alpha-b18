...
#ifndef PREFERRED_DEBUGGING_TYPE /* do what he said... */
...
#endif
#endif /* PREFERRED_DEBUGGING_TYPE */
...
char *(*decl_printable_name) PROTO((tree decl, int verbosity));
...
/* END CYGNUS LOCAL sac/win32 */
  ...
  if (quiet_flag)
    return 0;
/* CYGNUS LOCAL sac/win32 */
#if (defined(_WIN32)&& !defined__OPENNT) && !defined (HAVE_RUSAGE)
/* END CYGNUS LOCAL sac/win32 */
  if (clock() < 0)
    return 0;
  else
...
#ifdef USG
  times (&tms);
  return (tms.tms_utime + tms.tms_stime) * (1000000 / CLOCKS_PER_SEC);
#else
#ifndef VMS
  getrusage (0, &rusage);
...
void
real_fatal_insn (message, insn, line)
     char *message;
     rtx insn;
     int line;
{
fprintf(stderr, "fatal_insn in toplev, line %d\n", line);
  if (!output_bytecode)
    {
      error (message);
fprintf(stderr, "------Failing expression\n");
      debug_rtx (insn);
fprintf(stderr, "------Context\n");
print_rtl (stderr, PREV_INSN(PREV_INSN(PREV_INSN(PREV_INSN(insn)))));
    }
  if (asm_out_file)
    fflush (asm_out_file);
  if (aux_info_file)
...
void
real_fatal_insn_not_found (insn, file, line)
     rtx insn;
     char *file;
     int line;
{
  fprintf(stderr,"From gcc internal %s line %d\n", file, line);
  if (INSN_CODE (insn) < 0)
  ...
  while (na > input_name)
    {
      if (na[-1] == '/')
	break;
      /* CYGNUS LOCAL -- meissner/PR 11795 */
#ifdef DIR_SEPARATOR
      if (na[-1] == DIR_SEPARATOR)
	break;
#endif
      /* END CYGNUS LOCAL -- meissner/PR 11795 */
      na--;
    }
...
#if !defined(OS2) && !defined(VMS) && (!defined(_WIN32) || defined (__CYGWIN32__)) && !defined(__OPENNT)
  if (flag_print_mem)
    {
