     ...
     enum machine_mode mode;
{
 ...
 repeat:
  ...
  switch (code)
    ...
    case LABEL_REF:
      hash
	+= ((unsigned) LABEL_REF << 7) + (host_ptr_as_int) XEXP (x, 0);
      return hash;

    case SYMBOL_REF:
      hash
	+= ((unsigned) SYMBOL_REF << 7) + (host_ptr_as_int) XSTR (x, 0);
      return hash;
	...
	if (base && GET_CODE (base) == LABEL_REF)
	  {
		...
		if (offset >= 0
			...
			< XVECLEN (table, 1)))
		  {
		    ...
		    /*  We need to make sure the reference is to the
			right type, but we can't assume much about the
			relative sizes of the table and Pmode.  (In
			particular, NT/Alpha with 32-bit pointers and
			64-bit registers confuses things).  In particular
			a truncate only applies if narrowing from the table. */

		    if (GET_MODE_SIZE(GET_MODE(table) < GET_MODE_SIZE(Pmode)))
		      {
#ifdef POINTERS_EXTEND_UNSIGNED
			if (POINTERS_EXTEND_UNSIGNED)
			  new = gen_rtx (ZERO_EXTEND, GET_MODE (table), new);
			else
			  new = gen_rtx (SIGN_EXTEND, GET_MODE (table), new);
#else
			  new = gen_rtx (SIGN_EXTEND, GET_MODE (table), new);
#endif
		      }
		    else if (GET_MODE_SIZE(GET_MODE(table) > GET_MODE_SIZE(Pmode)))
