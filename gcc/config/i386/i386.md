  ...
  "*
{
  if (NON_STACK_REG_P (operands[1]))
    ...
    /* if we're in 53-bit mantissa mode and using TFs, this is the least
       of our worries, and it works right in 60-bit mode; see the next insn
       for more details */
  ...
  "*
{
  if (NON_STACK_REG_P (operands[1]))
    ...
    {
      /* For the to remaining DI input modes, we must route the result thru
