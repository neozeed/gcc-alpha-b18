...
;; The use of $27 for NT (and the other OSs as well) is discussed in
;; alpha.h.  For NT, we don't need to insist on $27, but if we 
;; must register-ize something, $27 is a good choice.  Since by this
;; time, any non-saved registers (such as $27) have been saved, it's
...
;; Contrary to prior comments, it is necessary to probe the stack
;; for every increment done by alloca.  The reason is that a series
;; of alloca() calls can add up to more than a page's worth, and
;; there's no guarantee of a probe to get the stack allocated
;; between the alloca() calls.
;; cse_basic_block() is a real example of this (when compiling cccp.c).
;; Altho possible, it's more than I care to take on to determine when 
;; the accumulated alloca()s warrant a probe.

;; We also assume that explow.c has guaranteed us we're not doing alloca(0).
;; If we were, then the probe would probe unallocated memory (and in
;; fact the effect is to probe, and thus zero, the return address!)

;; Lastly, since we're guaranteed to have to probe the final address of
;; the allocation, the 4K (1/2 page) rule doesn't apply; we just step
;; thru whole pages until we're within a page of the final size, then
;; do the last probe to be sure we're not on the edge of a page.
  ...
  if (GET_CODE (operands[0]) == CONST_INT
	   && INTVAL (operands[0]) < 32768)
    {
	 ...
	 probes.  Then we update the stack by the constant.  */
      ...
      /* Assume the current SP value has been probed (or otherwise accessed)
	 Probe out in whole pages, and then the remainder */

      int count = INTVAL (operands[0]) / 8192;
      int probed = 8192;
  ...
  else
    {
      ...
      emit_insn (gen_adddi3 (tmp, stack_pointer_rtx, GEN_INT (-8192)));

      /* any const-ints's are >32K by test above, so we don't need this */
      if (GET_CODE (operands[0]) != CONST_INT)
	{
