     ...
     enum machine_mode mode;
{
  ...
  /* Some configurations insist on $27 for indirect calls (implying
     and extra move sometimes, even in the case of non-thunk calls).
     For NT (and apparently Open VMS), that's not needed (this gcc doesn't.)
     We can't insist on 27 until after a real reg has been assigned! */
     ...
     int n;
{
  ...
  /* FIXME: the comment below is patently false (see alpha.md!).  However,
     it's probably benign: HI and QI values will already have the sign
     bit extended properly to (at least) 32 bits, and the code below will 
     carry it to 64 if needed.  (This affects some other tests below, as well.)
  */
#if HOST_BITS_PER_WIDE_INT == 64
  ...
  /* The assertion that we don't get unsigned is false.  We do.
     (The test suite shows failures because of that if we compile
     with 32-bit HOST_WIDE_INTEGERs.)  The problem is that constant 
     folding (at least thru immed_double_const) throws away the mode
     of the constant because it's in a register.

     Problem ignored by using 64-bit integer type. */
  ...
  /* If we couldn't do it that way, try some other methods.  But if we have
     no instructions left, don't bother.  Likewise, if this is SImode 
     (or narrower) and we can't make pseudos, we can't do anything since 
     the expand_binop and expand_unop calls will widen and try to make 
     pseudos.  */

  if (n == 1
      || (mode != DImode && ! rtx_equal_function_value_matters))
    return 0;
  ...
  for (i = 1; i < n; i++)
	...
	for (; bits > 0; bits--)
	 ...
	 mode and to avoid shifting outside the host wide int size.  */
      ...
      /* floor_log2() yields the number of bits you have to shift
	 the (unsigned) value right to convert the value to zero,
	 less 1.  That is, 0x3 yields 1, 0x4 yields 2; 0 yields -1.
	 0xffffffff yields 31.  0x7fffffff yields 30.
	 When shifting left, we don't want to shift far enough to
	 change the sign on a 32-bit host because in the unsigned
	 case we get a different value.  bits is the number of
	 high-order zero bits that might be shifted out.
	 We don't know if c is intended to be unsigned, so have 
	 to be conservative.

	 A more extensive modification would be to determine if c
	 is intended to be unsigned and allow sign extension if it
	 is.  It would also help to know the mode of the constant
	 (SI or DI); that was lost earlier (in immed_double_const
	 where it converts the constant to VOIDmode. (GEN_INT call
	 near line 2132).  (There's a problem with that; there's
	 no entry in alpha.md corresponding to what's generated
	 when setting mode to SI.  Doing it right may yield another
	 path that is OK, or we may need another rule for ZAPing
	 the high part of a register; however, the unsigned-ness
	 fix is needed anyway
      */
      ...
      if (bits > 0)
	for (; bits > 0; bits--)
	/* Possible further optimization: if c<<bits is <0, shift in 1's from
	   the right for the first choice (in the hope of getting a -1) */
...
#if !defined(CROSS_COMPILE) && !defined(_WIN32) && !defined(__linux__) && !defined(VMS) && !defined(__OPENNT)
#include <stamp.h>
#endif
     ...
     int size;
{
...
#if defined (WINNT)
  /* The macros don't have a decent fix for this one */
  alpha_auto_offset = -frame_size;
#else
  ...
  if (TARGET_WINDOWS_NT == 0)
    ...
    {
      alpha_function_needs_gp = 0;  /* so we don't report it later */
    }
...
#if defined (WINNT)
      /* For NT stack unwind (reverse execution), it's not kosher to take the
	 result of a loop, so even tho the value is already in $4
	 (+- leftover) we reload it via a single lda (which the assembler
	 will invariably convert to ldah/lda) and add it to sp. */

      fprintf (file, "\tlda $4,-%d\n", frame_size);
      fprintf (file, "\taddq $30,$4,$30\n");
#else
...
{
  struct {
    unsigned int i     : 32;	/* Mask of int regs */
    unsigned int fp    : 32;	/* Mask of fp regs */
