...
/* We don't have any switched on the mn10200.  Though there are some things
   ...
   -momit-frame-pointer to eliminate as many frame pointers as possible,
   even if doing so makes debugging impossible.  */
...
#define REG_OK_FOR_INDEX_P(X)  \
  (GET_MODE (X) == PSImode && ((REGNO (X) >= 0 && REGNO(X) <= 3) || REGNO (X) >= FIRST_PSEUDO_REGISTER))
/* Nonzero if X is a hard reg that can be used as a base reg
   or if it is a pseudo reg.  */
#define REG_OK_FOR_BASE_P(X) \
  (GET_MODE (X) == PSImode && ((REGNO (X) >= 4 && REGNO(X) <= 8) || REGNO (X) >= FIRST_PSEUDO_REGISTER))
#else
/* Nonzero if X is a hard reg that can be used as an index.  */
#define REG_OK_FOR_INDEX_P(X) (GET_MODE (X) == PSImode) && REGNO_OK_FOR_INDEX_P (REGNO (X))
/* Nonzero if X is a hard reg that can be used as a base reg.  */
#define REG_OK_FOR_BASE_P(X) (GET_MODE (X) == PSImode) && REGNO_OK_FOR_BASE_P (REGNO (X))
#endif
