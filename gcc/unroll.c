...
static int check_reg_usage (rtx);
static void reg_note_cleanup (rtx);
...
/* Walk the expression x; if a dead register is found, return false.
   If no dead registers, return true.  */
     ...
     rtx x;
{
  ...
  switch (code = GET_CODE (x))
    {
    case REG:
      return REGNO(x) >= max_reg_before_loop 
	 || (regno_last_uid[REGNO(x)] - regno_first_uid[REGNO(x)]) != 0;
    ...
    case SET:
      if (!check_reg_usage (SET_DEST (x))) return 0;
      return check_reg_usage (SET_SRC (x));
    ...
    case EXPR_LIST:
    case INSN_LIST:
      /* we're already parsing a note... it shouldn't be one of these */
      abort();
    }
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e')
	if (!check_reg_usage (XEXP (x, i))) return 0;
      else if (fmt[i] == 'E')
	for (j = XVECLEN (x, i) - 1; j >= 0; j--)
	  if (!check_reg_usage (XVECEXP (x, i, j))) return 0;
    }
  return 1;
}

/* Some register notes should not be copied, because they'll confuse
   CSE.  Specifically, those referring to a register which is now dead.
   Elimiate REG_EQUAL's with the offending registers from the list.

   A possibly better way of achieving the same end is to substitute
   regno_last_note_uid for regno_last_uid in some/many of the uses
   in loop.c, here, and cse.c.  It will take more analysis than is
   feasable now to determine which such substitutions are valid and
   which are not.  (That does not necessarily mean that this code isn't
   useful.)  (See jump.c for uses of regno_last_note_uid; the same
   problem appears to have occurred there.)

   (The actual problem that shows up if this is not done is detected
   by the regressions.  What happens is that the note is retained,
   but the set of the register it depends upon is discarded (if
   the note was the only use).  Later in unrolling/cse-ing, it uses
   that now dead register, concludes that the value of the register
   is zero (not detecting an error!) and generates incorrect code.
   (cse's use of src_eqv and its equivalence class.) (There should
   be a check for live register at that point.) */

static void
reg_note_cleanup (insn)
     rtx insn;
{
  rtx link, first;
  rtx *prev;
  ...
  while (link != NULL) 
    {
      if (GET_CODE (link) == EXPR_LIST && !check_reg_usage(XEXP(link,0)))
	{
	   link = XEXP(link,1);
	}
      else
