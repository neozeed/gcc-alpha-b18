	  ...
	  && ! may_not_optimize[REGNO (SET_DEST (set))])
	{
	  ...
	  if (REGNO (dest_reg) < FIRST_PSEUDO_REGISTER)
	    continue;
          ...
          /* BOTH the SET and the REG_EQUAL may be givs, so we have
	     to do this twice (think "generator" below)*/
	  for (t=1;t<=2;t++) 
	    {
	      switch(t)
		{
  ...
  switch (code)
      ...
      if (CONSTANT_P (arg))
	{
	  ...
	  for (v = bl->giv; v; v = v->next_iv)
		...
		/* We're abusing emit_iv_add_mult() a little here by handing
		   it the limit, rather than the step... it could overflow.

		   If the expression will overflow because of the multiplier
		   we don't want to make this substitution; at the same time
		   we check to see if it overflows HOST_WIDE_INT, because if
		   it does, we can't continue the substitution reliably .

		   note: floor_log2 returns one smaller than we need here */
		...
		if (GET_CODE (arg) == CONST_INT
		    && GET_CODE (v->mult_val) == CONST_INT)
		  {
		    int w;
		    HOST_WIDE_INT t, s;
		    w = floor_log2(INTVAL(arg))
		        + floor_log2(INTVAL(v->mult_val)) + 2;
		    if (w > GET_MODE_BITSIZE(mode) 
		       || w > HOST_BITS_PER_WIDE_INT)
			continue;

		    /* if the addend plus the multiply (which now can't overflow
		       on us) overflows, we don't want to do this either.
		       There's the possibility that the addend is an address;
		       we can't do a thing about the possibility of overflow
		       after relocation (except expect that in the real world
		       it won't happen) */

		    if (GET_CODE (v->add_val) == CONST_INT)
		     { 
			t = INTVAL(arg) * INTVAL(v->mult_val);

			/* macro below stolen from expmed.c; see there */
#define overflow_sum_sign(a, b, sum) ((~((a) ^ (b)) & ((a) ^ (sum))) < 0)
			s = t + INTVAL(v->add_val);
			if (overflow_sum_sign(INTVAL(v->add_val),t,s))
			    continue;
		     }
		  }
