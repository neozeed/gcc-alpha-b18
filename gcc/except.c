     ...
     rtx context;
{
  ...
  convert_move (eh_saved_pc_rtx, context, 1);
  emit_throw ();
}
...
expand_leftover_cleanups ()
{
  ...
  while ((entry = dequeue_eh_entry (&ehqueue)) != 0)
	...
	abort ();
      ...
      /* CYGNUS LOCAL EH/law */
      /* The exception handler label is (effectively) the target
	 of a nonlocal goto.  Mark it as such.  */
...
end_eh_unwinder ()
{
  ...
  /* Get the address we need to use to determine what exception
     ...
     It may be necessary to convert type (for NT), so use
     convert_move() */
