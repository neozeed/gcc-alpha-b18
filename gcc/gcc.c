...
struct option_map option_map[] =
 {
   {"--all-warnings", "-Wall", 0, "Generate all possible warnings"},
   {"--ansi", "-ansi", 0, "ANSI Standard C"},
   {"--assemble", "-S", 0, "Generate assembler as <file>.s" },
   {"--assert", "-A", "a", "Assert conditions about compile environment." },
   {"--comments", "-C", 0, "Retain comments thru preprocessor"},
   {"--compile", "-c", 0, "Compile only (do not link)"},
   {"--debug", "-g", "oj", "Record debugging information (arg permitted)"},
   {"--define-macro", "-D", "aj","Define Macro that is arg"},
   {"--dependencies", "-M", 0, "Output make dependency rules for files compiled"}, 
   {"--dump", "-d", "a", "Dump intermediate compiler passes (arg required)"},
   {"--dumpbase", "-dumpbase", "a", "File basename for -d output"},
   {"--entry", "-e", 0, "Specify program entry (see ld)"},
   {"--extra-warnings", "-W", 0, "Generate more detailed warnings"},
   {"--for-assembler", "-Wa", "a","Pass argument to assembler (use as --help)"},
   {"--for-linker", "-Xlinker", "a", "Pass argument to linker (use ld --help)"},
   {"--force-link", "-u", "a", "Add arg as undef symbol (see ld -u)"},
   {"--help", "-help", 0, "Show this message and exit."},
   {"--imacros", "-imacros", "a", "Process file <arg> retaining only macro defs"},
   {"--include", "-include", "a", "Process file <arg> as if included before 1st line"},
   {"--include-barrier", "-I-", 0, "Do not use prior -I's when searching for \"file\""},
   {"--include-directory", "-I", "aj", "Specify include search directory"},
   {"--include-directory-after", "-idirafter", "a", "Add to secondary include list."},
   {"--include-prefix", "-iprefix", "a", "Prefix for -iwithprefix..."},
   {"--include-with-prefix", "-iwithprefix", "a", "Add prefix/name to secondary include list"},
   {"--include-with-prefix-before", "-iwithprefixbefore", "a", "Add prefix/name to primary include list."},
   {"--include-with-prefix-after", "-iwithprefix", "a", "Add prefix/name to secondary include list"},
   {"--language", "-x", "a", "Force input language for following filenames"},
   {"--library-directory", "-L", "a", "Specify library search directory"},
   {"--machine", "-m", "aj", "Set target machine to compile for"},
   {"--machine-", "-m", "*j", "Set target machine to compile for"},
   {"--no-line-commands", "-P", 0, "With -E: generate no #lines"},
   {"--no-precompiled-includes", "-noprecomp", 0, "Always true pending FSF completing implementation."},
   {"--no-standard-includes", "-nostdinc", 0, "Omit standard places from #include search"},
   {"--no-standard-libraries", "-nostdlib", 0, "Omit standard places from lib search."},
   {"--no-warnings", "-w", 0, "Inhibit all warning messages."},
   {"--optimize", "-O", "oj", "Set optimization level."},
   {"--output", "-o", "a", "Name output file (including -E, -c, or link)"},
   {"--pedantic", "-pedantic", 0, "Complain about benign extensions to ANSI."},
   {"--pedantic-errors", "-pedantic-errors", 0, "Fatal error on benign extensions."},
   {"--pipe", "-pipe", 0, "Use pipes rather than temp files."},
   {"--prefix", "-B", "a", "Another place to look for compiler pieces."},
   {"--preprocess", "-E", 0, "Preprocessor only, output to stdout."},
   {"--print-search-dirs", "-print-search-dirs", 0, "Output search paths and exit"},
   {"--print-file-name", "-print-file-name=", "aj", "Print full name of lib arg and exit"},
   {"--print-libgcc-file-name", "-print-libgcc-file-name", 0, "Same as -print-file-name=libgcc.a"},
   {"--print-missing-file-dependencies", "-MG", 0, "-M but allow missing headers"},
   {"--print-multi-lib", "-print-multi-lib", 0, "Print multilib info and exit."},
   {"--print-multi-directory", "-print-multi-directory", 0, "Print multilib directory name and exit."},
   {"--print-prog-name", "-print-prog-name=", "aj", "Print full name of compiler phase and exit"},
   {"--profile", "-p", 0, "Add code to enable profiling"},
   {"--profile-blocks", "-a", 0, "Profile basic blocks."},
   {"--quiet", "-q", 0, "Work quietly."},
   {"--save-temps", "-save-temps", 0, "Crete <name>.i, <name>.s, <name>.o"},
   {"--shared", "-shared", 0, "Generate shared executable"},
   {"--silent", "-q", 0, "Work quietly"},
   {"--specs", "-specs=", "aj", "Use arg as specs (rules for compiler stages)"},
   {"--static", "-static", 0, "Link statically (no dynamic links)"},
   {"--symbolic", "-symbolic", 0, "Bind to globals when building shared lib"},
   {"--target", "-b", "a", "Target for cross compilation."},
   {"--trace-includes", "-H", 0, "Print name of each header included."},
   {"--traditional", "-traditional", 0, "Traditional (non-ANSI) C sematics"},
   {"--traditional-cpp", "-traditional-cpp", 0, "Tradional Cpp rules"},
   {"--trigraphs", "-trigraphs", 0, "Enable ANSI trigraphs"},
   {"--undefine-macro", "-U", "aj", "Undefine macro name."},
   {"--use-version", "-V", "a", "Select specific compiler version number."},
   {"--user-dependencies", "-MM", 0, "-M, but only user files"},
   {"--verbose", "-v", 0, "Describe stages of compilation to stdout"},
   {"--version", "-dumpversion", 0, "Print compiler version and exit."},
   {"--warn-", "-W", "*j", "Enable/disable specific warnings"},
   {"--write-dependencies", "-MD", 0, "Like -M, but write to .d and compile"},
   {"--write-user-dependencies", "-MMD", 0, "-MD but only user headers."},
   {"--", "-f", "*j", NULL}
 };

...
execute ()
{
  ...
  for (n_commands = 1, i = 0; i < argbuf_index; i++)
    if (strcmp (argbuf[i], "|") == 0)
      {				/* each command.  */
#if defined (__MSDOS__) || (defined (_WIN32) && ! defined (__CYGWIN32__) && !defined(__OPENNT)) || defined (OS2)
     ...
     char **argv;
{
...
#ifdef __CYGWIN32__
  char *win32temp;
  int win32_buf_size = 0; /* memory we need to alloca */
  int win32_alloca_size = 0; /* memory we have alloca'd */
#endif
  char *spec_lang = 0;
  ...
  if (gcc_exec_prefix == NULL) {
      gcc_exec_prefix = concat(getenv("OPENNT_ROOT"),"/usr/contrib/", NULL_PTR);
  }
  add_prefix (&startfile_prefixes, concat(getenv("OPENNT_ROOT"),"/usr/lib/",0), 0, 0, NULL_PTR);
#endif
  ...
  if (temp && cygwin32_posix_path_list_p (temp))
    {
      win32_buf_size = cygwin32_posix_to_win32_path_list_buf_size (temp);
      if (win32_buf_size > win32_alloca_size)
        win32temp = (char *) alloca (win32_buf_size);
      win32_alloca_size = win32_buf_size;
      cygwin32_posix_to_win32_path_list (temp, win32temp);
      temp = win32temp;
    }
#endif
  if (temp)
  ...
  for (i = 1; i < argc; i++)
      ...
      if (! strcmp (argv[i], "-help"))
	{
      ...
      else if (! strcmp (argv[i], "-dumpspecs"))
	{
	  struct spec_list *sl;
	  init_spec ();
	  for (sl = specs; sl; sl = sl->next)
     ...
     char **argv;
{
  ...
  for (i = 0; i < n_switches; i++)
    if (! switches[i].valid)
      error ("unrecognized option `-%s'; use --help", switches[i].part1);
    ...
    char *name;
{
    ...
    for (i = 0; i < sizeof (option_map) / sizeof (option_map[0]); i++) {
       if (option_map[i].help_text == NULL) 
	   continue;
       if (strcmp(&(option_map[i].name[1]), option_map[i].equivalent) != 0)
	 {
	   printf("  %-14s  ", option_map[i].name);
	   printf("%s", option_map[i].equivalent);
	   if (strlen(option_map[i].equivalent) + 
	       strlen(option_map[i].name) >= 21
	       || (strlen (option_map[i].equivalent) > 7)) 
	      printf("\n                          -- ");
	   else
	     {
	       l = MAX(0, (7-(int)strlen(option_map[i].equivalent)) -
		          MAX(0,(int)strlen(option_map[i].name)-14));
	       printf("%*.*s",l, l, "       ");
	       printf(" -- ");
	     }
	 }
	 ...
	 {
	   printf("  %-23s", option_map[i].name);
	   if (strlen(option_map[i].name) > 23) 
	      printf("\n                         ");
	   printf(" -- ");
	 }
       printf("%s\n", option_map[i].help_text);
    }
    printf("  -Tdata <address>        -- set address of .data section\n");
    printf("  -Ttext <address>        -- set address of .text section\n");
    printf("  -Tbss  <address>        -- set address of .bss  section\n");
    printf("  -aux-info <file>        -- write func prototypes to <file>\n");
    printf("  -isystem <dir>          -- add <dir> as system include dir\n");
    printf("  -ifoutput               -- output cpp conditionals w/ #failed\n");
    printf("  -dumpspecs              -- Dump current specs and exit.\n");
    printf("  -dumpmachine            -- print current machine name and exit.\n");
    printf("  -dumpmachine            -- ??\n");
    printf("                  -Wp,    -- Send args to preprocessor\n");
    printf("                  -Wl,    -- Send args to linker\n");
    printf("                  +e,     -- Send args to C++ front end (cfront)\n");
    printf("  -f<name>                -- flag to compiler proper\n");

    printf("\nArguments (this list can be modified on a site-dependent basis; the default):\n");
    printf("  <name>.c                --  compile C\n");
    printf("  <name>.h                --  compile C header\n");
    printf("  <name>.i                --  compile cpp output as C\n");
    printf("  <name>.s                --  assemble\n");
    printf("  <name>.S                --  assemble through cpp\n");
    printf("  <name>.{cc|cxx|c++|cpp|C} --  compile C++ if compiler installed\n");
    printf("  <name>.m                --  compile Objective-C if compiler installed\n");
    printf("  <name>.{ads|adb|ada}    --  compile Ada if compiler installed\n");
    printf("  <name>.{f|F|for|fpp}    --  compile Fortran if compiler installed\n");
    printf("  <name>.o                --  link object file\n");
    printf("  <name>.a                --  link library (by that filename)\n");
    printf("  -l<name>                --  search lib lib<name>.a (as specified by -L)\n");
    printf("  other name              --  pass on to linker\n");

    printf("\nUse -Wl,-M to get a link map\n");
    printf("Use -g[gdb|[x]coff[+]|stabs[+]|dwarf[+|-2]] to control debug format.\n");
    printf("   ... not all possibilities are supported.\n");

    printf("\nThere are additional options and not all options work on\n"
	     "all configurations and systems.  See additional documentation\n"
	     "for details.\n");
    printf("\nReport bugs to bug-gcc@prep.ai.mit.edu\n");
}
