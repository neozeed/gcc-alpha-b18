/* Search an insn for pseudo regs that must be in hard regs and are not.
   Copyright (C) 1987, 88, 89, 92-7, 1997 Free Software Foundation, Inc.
...
static int find_reloads_address	PROTO((enum machine_mode, rtx *, rtx, rtx *,
				       int, enum reload_type, int, rtx, short *));
     ...
     register rtx x, y;
{
  ...
  if ((code == REG || (code == SUBREG && GET_CODE (SUBREG_REG (x)) == REG))
	...
	i += GET_MODE_REGISTER_SIZE(GET_MODE(x));
      if (WORDS_BIG_ENDIAN && GET_MODE_SIZE (GET_MODE (y)) > UNITS_PER_WORD
	  && j < FIRST_PSEUDO_REGISTER)
	j += GET_MODE_REGISTER_SIZE(GET_MODE(y));
 ...
 finish:
  ...
  for (i = 0; i < noperands; i++)
	...
	if (insn_operand_address_p[insn_code_number][i])
	  find_reloads_address (VOIDmode, NULL_PTR,
				recog_operand[i], recog_operand_loc[i],
				i, RELOAD_FOR_INPUT, ind_levels, insn, &ign);
	...
	find_reloads_address (GET_MODE (recog_operand[i]),
			      ...
			      XEXP (recog_operand[i], 0),
			      &XEXP (recog_operand[i], 0),
			      i, RELOAD_OTHER, ind_levels, insn, &ign);
      if (code == SUBREG)
	recog_operand[i] = *recog_operand_loc[i]
...
/*  This creates (subreg (mem...)) which would cause an unnecessary
  ...
  if (code == MEM)
    {
      ...
      find_reloads_address (GET_MODE (x), &tem, XEXP (x, 0), &XEXP (x, 0),
			    opnum, type, ind_levels, 0, force_update);
      return tem;
    }
  ...
  if (code == SUBREG && GET_CODE (SUBREG_REG (x)) == REG)
					...
					reg_equiv_constant[regno])) != 0)
	{
      ...
      /* We have to allow both the "normal" stuff, and the special case
	 of a SUBREG(REG(...)) where it's on a machine like NT Alpha: pointers
	 in registers are long words (DI) but pointers in memory are
	 ordinary (SI), and the equivalent constant is a memory address.
	 This case handles the normal case (see gperf.cc/NT Alpha);

	 Both this and the next case address the "Learn why..." aborts
	 above and below... SUBREG(REG(...)) sometimes ended up in the
	 recursive call (in the for loop) below, and that passes in a NULL
	 for force_updates.  Fixing the recursive call caused bad code
	 to be generated (such that g77 itself failed). */

      if (regno >= FIRST_PSEUDO_REGISTER && reg_renumber[regno] < 0
	  && reg_equiv_constant[regno] != 0
	  && (GET_MODE_BITSIZE (GET_MODE (x)) == BITS_PER_WORD
	     || (GET_MODE(SUBREG_REG(x)) == Pmode
	       && GET_MODE_BITSIZE(GET_MODE(x)) == POINTER_SIZE
	       && GET_CODE(reg_equiv_constant[regno]) == SYMBOL_REF))
	...
	tem = operand_subword (reg_equiv_constant[regno],
      ...
      /* As above, the paradoxical case (which proves to be simpler);
	 occurs in dlltool.c on NT Alpha */
      if (regno >= FIRST_PSEUDO_REGISTER && reg_renumber[regno] < 0
	  && reg_equiv_constant[regno] != 0
	  && GET_MODE(x) == Pmode
	  && GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x))) == POINTER_SIZE
	  && GET_CODE(reg_equiv_constant[regno]) == SYMBOL_REF)
	{
	  if (force_update)
	    *force_update = 1;
  ...
  if (GET_CODE (ad) == MEM)
    {
	 ...
	 indirect addresses are valid, reload the MEM into a register.  */
      ...
      find_reloads_address (GET_MODE (ad), &tem, XEXP (ad, 0), &XEXP (ad, 0),
			    ...
			    ind_levels == 0 ? 0 : ind_levels - 1, insn, NULL);
  ...
  switch (code)
      ...
      else if (GET_CODE (XEXP (x, 0)) == MEM)
	{
	  ...
	  find_reloads_address (GET_MODE (x), &XEXP (x, 0),
				...
				opnum, type, ind_levels, insn, NULL);
    ...
    case MEM:
	 ...
	 reload1.c here.  */
      ...
      find_reloads_address (GET_MODE (x), loc, XEXP (x, 0), &XEXP (x, 0),
			    opnum, ADDR_TYPE (type), ind_levels, insn, NULL);
