...
/* CYGNUS LOCAL mpw */
#ifdef MPW
/* These are reserved words for MPW.  */
#define comp comp_
...
/* CYGNUS LOCAL mpw */
#ifdef MPW_C
/* Unlikely-looking fix to a mystifying MPW C bug.  */
#define ret rlabel1
#endif
/* END CYGNUS LOCAL */
     ...
     int want_value;
{
  ...
  if (TREE_CODE (exp) == COMPOUND_EXPR)
	...
	/* was... temp = convert_modes (GET_MODE (SUBREG_REG (target)), */
...
#if 0
	  if (*pbitpos > INT_MAX/BITS_PER_UNIT || *pbitpos < 0) {
	      /* ??? If one were ambitious, one could allow huge offsets
		 by making pbitpos wider.  However, it's probably not
		 worth it because such objects probably couldn't actually
		 be allocated anyway */
	      error ("Bit offset of field or element too large to access");
	      *pbitpos = 0;
	  }
#endif
  ...
  switch (code)
      ...
      if (TREE_CODE (TREE_OPERAND (exp, 1)) == INTEGER_CST)
	{
	  ...
	  if (TREE_UNSIGNED (type) || TREE_OVERFLOW (negated))
	    {
	      tree newtype, newop0, newop1, newneg;

	      newtype = signed_type (type);
	      /* if signed_type() didn't do it, either it's a pointer and
		 we'll beat it into submission here, or we're in deep.... */
	      if (TREE_UNSIGNED(newtype)) 
		{
		  if (POINTER_TYPE_P(newtype)) 
		    {
		      newtype = type_for_size(POINTER_SIZE,0);
		      newop0 = TREE_OPERAND (exp, 0);
		      newop1 = convert (newtype, TREE_OPERAND (exp, 1));
		      newneg = fold (build1 (NEGATE_EXPR, newtype, newop1));
		    }
		    else
		      abort();
	        }
	        ...
	        {
		  newop0 = convert (newtype, TREE_OPERAND (exp, 0));
		  newop1 = convert (newtype, TREE_OPERAND (exp, 1));
		  newneg = fold (build1 (NEGATE_EXPR, newtype, newop1));
	        }
	  ...
	  if (modifier == EXPAND_SUM || modifier == EXPAND_INITIALIZER)
	    {
		  ...
		  GET_MODE (temp) == Pmode && GET_MODE (temp) != mode
		  && mode == ptr_mode)
...
#ifdef POINTERS_EXTEND_UNSIGNED
      if (GET_CODE(op0) != REG && 
	  GET_MODE (op0) == Pmode && GET_MODE (op0) != mode
	  && mode == ptr_mode)
     ...
     unsigned max_align;
{
  ...
  /* CYGNUS LOCAL unaligned-pointers & -fpack-struct */
  if (SLOW_UNALIGNED_ACCESS
      && (flag_unaligned_pointers || maximum_field_alignment != 0
	  || flag_pack_struct))
    align = BITS_PER_UNIT;
  /* END CYGNUS LOCAL */
     ...
     rtx tem;
{
  ...
  /* On (typically RISC) machines which try to be smart about their
     use of stack and frame pointers, chaining through the frame pointers
     won't work (often because there simply isn't one in a given function).
     It takes fairly sophisticated stack unwind routines (usually involving
     reverse execution of prologues or complex tables) to determine anything
     about a caller's frame, including finding the caller's parent's saved
     Return Address; it may even take a symbol table.  PA-RISC ("HPPA") and
     Alpha are both this way, with very different unwind mechanisms.
     __builtin_return_addr cannot work outside the current function (at least
     not without support routines not yet written) in these cases, so 
     generate a (fatal) diagnostic.   We need to support values of
     1 for the exception stuff for C++ (see cp/except.c).
     (If you do try to fix it for a given machine, don't forget about
     alloca()!).

     Additionally, at least for the Alpha, determination as to whether
     a FP will exist or not is scattered throughout the code, with the
     final determination being made at codegen time, depeinding on
     (optimized) register usage (see reload1.c).  Thus, we can't know how 
     to find the RA even for the current function at this time.  We could
     add a NOTE type to resolve it later, but rather the caller forces a
     FP here, which works in at least the required cases.

     FIXME: leaf functions with no/few locals may need to look directly 
     into the RA register!

     Ideally, this would be a syntax error at gcc compile time to inform 
     compiler builders that this may not work, but probably this will be
     sufficient. */
#if !defined(NONLEAF_ALWAYS_HAS_FP) && !defined(DYNAMIC_CHAIN_ADDRESS)
  if (count > 1)
    {
      error ("This architecture does not support '__builtin_return_address' with count > 1");
      return const0_rtx;
    }
#endif
     ...
     int ignore;
{
  ...
  switch (DECL_FUNCTION_CODE (fndecl))
      ...
      else
	{
	  ...
	  /* Say that we need a FP register; some RISCs may otherwise
	     discard it.  See more in expand_builtin_return_addr */
          emit_insn (gen_rtx (USE, VOIDmode, frame_pointer_rtx));
	  tem = expand_builtin_return_addr (DECL_FUNCTION_CODE (fndecl),
