     ...
     int indent;
{
  ...
  fprintf (file, HOST_PTR_PRINTF, (host_ptr_as_int) node);
     ...
     int indent;
{
  ...
  tc = TREE_CODE(node);
  if (tc > NUM_TREE_CODES || tc < 0)
    {
       fprintf(file, "Bad code %d\n", tc);
       return;
    }

  class = TREE_CODE_CLASS (tc);
  ...
  hash = ((host_ptr_as_int) node) % HASH_SIZE;
	  ...
	  if (TREE_CODE (node) == PARM_DECL)
	  ...
	  else if (TREE_CODE (node) == FUNCTION_DECL)
	    {
	      ...
	      fprintf (file, HOST_PTR_PRINTF,
 		       (host_ptr_as_int) DECL_SAVED_INSNS (node));
	    }
	}
