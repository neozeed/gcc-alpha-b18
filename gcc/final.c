...
static int last_linenum = 1;   /* smallest legal value */
...
#if defined(SDB_DEBUGGING_INFO) && !defined(SDB_DELAY_ARGS_INFO)
     ...
     int nopeepholes;
{
  ...
  switch (GET_CODE (insn))
	...
	{
#if defined (DBX_DEBUGGING_INFO) && defined(SDB_DELAY_ARGS_INFO)
	  if (write_symbols == DBX_DEBUG)
	      dbxout_parms (DECL_ARGUMENTS (current_function_decl));
	  else
#if defined(SDB_DEBUGGING_INFO) && defined(SDB_DELAY_ARGS_INFO)
	  /* MIPS (and Alpha) stabs require the parameter descriptions to be 
	     after the function entry point rather than before.  */
#endif
	  if (write_symbols == SDB_DEBUG)
	    sdbout_begin_function (last_linenum);
	  else
