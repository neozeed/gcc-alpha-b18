...
/^[ 	]*if \[ "{SYSTEM_HEADER_DIR}" = "{tooldir}:sys-include" \]/,/^[ 	]*End If/d
...
/^LANGUAGES/s/gcov//
# Nor other weird tools.
/^LANGUAGES.*depragma/s/^/#/
...
# Fix up sed commands victimized by other edits.
/sed -e /s/\\Option-x/*/g
/sed -e /s/{}/$/g
...
# Use the target includes we defined above.
/{GCC_FOR_TARGET}/s/ {INCLUDES}/ {INCLUDES_FOR_TARGET}/g

# Remove recursive make of extra libgcc2 functions.
/\.txt" == /,/^[ 	]*else/c\
	If 0
...
# Remove unused install actions.
/^install-dir \\Option-f /,/^$/d
/^install-prog \\Option-f /,/^$/d
/^install-headers \\Option-f /,/^$/d

# Replace the Unix install action with an MPW rendition.
/^install \\Option-f /,/^$/c\
...
install-dir \\Option-f\
	...
	NewFolderRecursive "{libdir}"\
	NewFolderRecursive "{libdir}gcc-lib:"\
	NewFolderRecursive "{libdir}gcc-lib:{target_canonical}"\
	NewFolderRecursive "{libsubdir}"\
	NewFolderRecursive "{libsubdir}include:"\
\
install-prog \\Option-f\
	sed -e '/xgcc/s/xgcc/gcc/' "{o}"xgC > "{bindir}"gC\
	...
	Duplicate -y "{o}"xgcc "{bindir}"gcc\
	# (should) Install elements of the gcc library.\
	Duplicate -y "{o}"cpp "{libsubdir}"cpp\
	Duplicate -y "{o}"cc1 "{libsubdir}"cc1\
	If "`Exists "{o}"cc1plus`" != ""\
		Duplicate -y "{o}"cc1plus "{libsubdir}"cc1plus\
	End If\
	"{o}"xgcc -dumpspecs > "{libsubdir}"specs\
	Duplicate -y "{o}"libgcc.a "{libsubdir}"libgcc.a\
	If "`Exists "{o}"libgcc.xcoff`" != ""\
		Duplicate -y "{o}"libgcc.xcoff "{libdir}"libgcc.xcoff\
	End If\
\
...
\
"{o}"libgcc.xcoff \\Option-f "{o}"libgcc.a\
   "{o}"ld --whole-archive -r "{o}"libgcc.a -o "{o}"libgcc.xcoff\
\
