...
/* CYGNUS LOCAL mpw */
#if (defined (__STDC__) || defined (ALMOST_STDC)) && defined (HAVE_VPRINTF)
/* END CYGNUS LOCAL */
...
/* CYGNUS LOCAL joern/go32 */
/* Nonzero if inodes are known to match; zero if unequal or unknown.  */
/* END CYGNUS LOCAL */
...
/* CYGNUS LOCAL joern/go32 */
/* In the go32 libraries in newlib, fstat lacks inode support.  It is unknown
   if the current version of the go32 libraries has the same problem.  */
#ifdef __MSDOS__
/* Nonzero if inodes are known to match; zero if unequal or unknown.  */
...
#ifdef __OPENNT
char standard_include_buf[PATH_MAX];
char gplusplus_include_buf[PATH_MAX];
#endif
...
static void cleanup_dos_files PROTO((FILE_BUF *fp));

     ...
     char **argv;
{
...
#ifdef __OPENNT
    /* we do this early because the code check for directory existance later */
    if ( (_prefixInstallPath("/usr/include",
	    standard_include_buf, PATH_MAX - 1)) == (char *)NULL) {
	strcpy(standard_include_buf, "/OpenNT/usr/include");
    }
    if ( (_prefixInstallPath("/usr/include/g++",
	    gplusplus_include_buf, PATH_MAX - 1)) == (char *)NULL) {
	strcpy(gplusplus_include_buf, "/OpenNT/usr/include/g++");
    }
#endif
  ...
  for (i = 1; i < argc; i++) {
	...
	/* CYGNUS LOCAL mpw */
#ifdef MPW
	/* Recognize include options of the form -i <name>, for better
	   compatibility with MPW makefiles.  */
	if (!strcmp (argv[i], "-i")) {
	  if (i + 1 == argc)
	  ...
	  else
	    {
	      ...
	      if (! ignore_srcdir && !strcmp (argv[i + 1], "-")) {
		ignore_srcdir = 1;
		/* Don't use any preceding -I directories for #include <...>.  */
		first_bracket_include = 0;
	      }
	      else {
		dirtmp = new_include_prefix (last_include, "", argv[++i]);
      ...
      case 'd':
	{
	  ...
	  /* CYGNUS LOCAL mpw */
#ifdef MPW
	  /* Understand definitions of the form "-d <defn>", for better
	     compatibility with MPW makefiles.  */
	  if (*p == '\0') {
	    if (i + 1 == argc)
  ...
  /* CYGNUS LOCAL mpw */
#ifdef MPW
  /* Add dirs from GCCIncludes after dirs from -I and -i.  The effect
     is similar to what MPW C does with CIncludes and Metrowerks with
     MWCIncludes, but we use a different variable so that GCC need not
     look at other compilers' include files.  */
  cp = getenv ("GCCincludes");
  if (cp && ! no_standard_includes)
    path_include (cp);
#endif /* MPW */
  /* END CYGNUS LOCAL */
      ...
      if (cygwin32_posix_path_list_p (epath))
        {
          ...
          win32epath = (char *) alloca (win32_buf_size);
     ...
     char *fname;
{
  char *s = fname;
  char *p;
#if defined (__MSDOS__) || (defined (_WIN32) && !defined(__OPENNT))
...
absolute_filename (filename)
     char *filename;
{
#if defined (__MSDOS__) || (defined (_WIN32) && !defined (__CYGWIN32__) && !defined(__OPENNT))
...
/* CYGNUS LOCAL mpw */
/* Don't do filename mapping for MPW, causes crashes.  */
#ifdef MPW
  return filename;
#endif /* MPW */
/* END CYGNUS LOCAL */
     ...
     FILE_BUF *op;
{
  ...
  while (nstrings--) {
    ...
    if ((HOST_WIDE_INT) (long)cp & 3)
      cp += 4 - ((HOST_WIDE_INT) (long)cp & 3);
...
/* CYGNUS LOCAL mpw */
#ifdef MPW
    /* Write out a shell command that can be used to go to the error.  */
    fprintf (stderr, "File \"%s\"; Line %d  # ", ip->nominal_fname, ip->lineno);
#else /* not MPW */
/* END CYGNUS LOCAL */
...
static void cleanup_dos_files(fp)
FILE_BUF *fp;
{
    unsigned char *p = fp->buf; 
    unsigned char *q;
    unsigned char *bufend = &fp->buf[fp->length];
    ...
    /* we're pointing at the first newline */
    if (p[-1] != '\r') return;

    /* well... we have a dos-style file.  Given the assumptions made
       in the rest of the code that there is just a single-character
       line separator (particularly when dancing around escaped newlines
       and macros) we'll compress the CRLF pairs to just NL.

       Note... there may well be a way to leverage the use of newline as 
       an escape to deal with ths but it still requires some action here.
       (Minimally, converting CRLF into NLx, where x is treated specially,
       (like NL-) may work.  However, for the moment...

       There's also a potential issue that some application somewhere
       (compiled in GCC, mind you) might expect a CRLF imbedded literally
       (not with escapes) in a multi-line string to survive as a CRLF.
       I'm not going to worry about that and if it happens, recode
       it as \r\n.  Given that any typical DOS->Unix file formatter would
       have the same effect....
    */
    p = q = p-1;  /* might as well pick up where we left off... */
    while (p<bufend) {
        if (p[0] == '\r' && p[1] == '\n') p++;  /* skip CR */
        else if (p[0] == ('Z'& 037)) {
	    /* EOF character */
