     ...
     register rtx x;
{
  ...
  if (GET_CODE (x) == CONST_DOUBLE
  ...
  else if (GET_CODE (x) == REG)
    {
      ...
      if (! WORDS_BIG_ENDIAN)
	  word = GET_MODE_REGISTER_SIZE(GET_MODE(x)) 
...
/* CYGNUS LOCAL EH/law */
#ifdef HAVE_nonlocal_goto_prologue
      /* Emit any code required by the target machine at a
	 label which may be the target of a nonlocal goto.  */
      if (LABEL_PRESERVE_P (label)
	  && HAVE_nonlocal_goto_prologue)
	emit_insn_after (gen_nonlocal_goto_prologue (), label);
#endif
/* END CYGNUS LOCAL */
    }
...
/* CYGNUS LOCAL EH/law */
	 ...
	 label which may be the target of a nonlocal goto.  */
      if (LABEL_PRESERVE_P (label)
	  && HAVE_nonlocal_goto_prologue)
	emit_insn (gen_nonlocal_goto_prologue ());
#endif
/* END CYGNUS LOCAL */
    }
  return label;
