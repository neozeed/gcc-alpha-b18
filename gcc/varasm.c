     ...
     int dont_output_data;
{
  ...
  if ((DECL_INITIAL (decl) == 0 || DECL_INITIAL (decl) == error_mark_node)
      /* CYGNUS LOCAL gavin/sections for uninitialize vars */
      /* Don't use bss if target supports attribute section and decl wants
         to be in a specific section.  */
#ifdef ASM_OUTPUT_SECTION_NAME
      && DECL_SECTION_NAME (decl) == NULL_TREE
#endif
      /* END CYGNUS LOCAL gavin/sections for uninitialize vars */
  ...
  if (GET_MODE_CLASS (mode) == MODE_INT
	...
	i0 |= ((HOST_WIDE_INT) (-1) << width);
	 ...
	 is being broken.  */
      ...
      /* The problem above causes regression test failures on the alpha....
	 The simplest solution is to use 64-bit integer types.
      */
  ...
  switch (code)
      ...
      else
	{
	  ...
	  if (TREE_CODE (TREE_TYPE (exp)) == RECORD_TYPE)
	    hi = ((host_ptr_as_int) TREE_TYPE (exp) & ((1 << HASHBITS) - 1))
	      % MAX_HASH_TABLE;
	  else
...
#define SYMHASH(LABEL)	\
  ((((host_ptr_as_int) (LABEL)) & ((1 << HASHBITS) - 1))  % MAX_RTX_HASH_TABLE)

