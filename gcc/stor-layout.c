...
/* G_B_F_A means that a change in the required alignment of a sequence
   ...
   tightly packed), prevails. */
#endif

/* The runtime test for whether a bitfield alignment is being done 
   in the code below might not be appropriate everywhere (in particular
   if TYPE_NATIVE is being tested).  The code will use the value of
   P_B_T_M unless P_B_T_T is defined; if it is, P_B_T_T will be used
   for the runtime test.  This relies on the variables "field" and
   "rec" to be (properly) meaningful in the context of use. */
   
#if defined(PCC_BITFIELD_TYPE_MATTERS) && !defined(PCC_BITFIELD_TYPE_TEST)
#define PCC_BITFIELD_TYPE_TEST PCC_BITFIELD_TYPE_MATTERS
#endif
static tree
layout_record (rec)
     tree rec;
...
#else
      if (PCC_BITFIELD_TYPE_TEST && TREE_TYPE (field) != error_mark_node
  ...
  for (field = TYPE_FIELDS (rec); field; field = TREE_CHAIN (field))
	...
	continue;
	 ...
	 forces the entire union to have `int' alignment.  */
      if (PCC_BITFIELD_TYPE_TEST && DECL_BIT_FIELD_TYPE (field))
