...
#ifdef POINTERS_EXTEND_UNSIGNED
      /* There's one other possibility: this is a pointer already
	 sitting in a register wider than ptr_type (that is, 
	 in Pmode).  We don't want to try to narrow the pointer
	 while it's in a register.  (No instructions available!) */

     if (POINTER_TYPE_P(TREE_TYPE(DECL_RESULT (fndecl)))
         && POINTER_TYPE_P(type))
	 arriving_mode = Pmode;
