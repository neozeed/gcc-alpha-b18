...
/*    REGNO (entry_parm);*/
  ...
  if (obey_regdecls)
  ...
  else
    {
	 ...
	 and write some of the results to dump file.  */
      ...
      /* The following should always come up empty, but who knows
	 for sure.  However, it must be called to clear leftovers
	 from the prior function. The other similar variables
	 instpected by flow_analysis seem to be being cleared 
	 during function setup */
  ...
  assemble_start_function (thunk_fndecl, fnname);
#ifdef ASM_DECLARE_THUNK_NAME
  ASM_DECLARE_THUNK_NAME(asm_out_file, fnname, current_function_decl);
#endif
