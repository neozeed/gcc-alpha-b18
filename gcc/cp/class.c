     ...
     tree virtuals, offset, type;
{
  ...
  if (! flag_vtable_thunks)
  ...
  else
    {
      ...
      TREE_VALUE (virtuals) = build_vtable_entry (signed_size_zero_node, voff);

      /* The second slot is for the tdesc pointer when thunks are used.  */
      TREE_VALUE (TREE_CHAIN (virtuals))
	= build_vtable_entry (signed_size_zero_node, vfn);
    }
}
  ...
  if (binfo)
    {
      ...
      offset = size_binop (MINUS_EXPR, signed_size_zero_node, offset);
      set_rtti_entry (virtuals, offset, type);
    }
  else
    {
     ...
     tree t; /* Structure type.  */
{
  ...
  if (DECL_VINDEX (fndecl) == error_mark_node)
	...
	DECL_CONTEXT (fndecl) = t;
      ...
      entry = build_vtable_entry (signed_size_zero_node, vfn);
  ...
  while (virtuals)
      ...
      if (fndecl)
	{
	  ...
	  base_offset = size_binop (PLUS_EXPR,
				    ...
				    BINFO_OFFSET (binfo));
          /* up to this point, the offsets are all positive (presumably).
	     However, this subtraction could make it negative, and if 
	     offset is unsigned, bad things can happen (because the
	     result of size_binop has the signed-ness of the first arg).
	     We can't just stuff the type because it might be (at least)
	     integer_zero_node (with nasty consequences later).   However,
	     we CAN assume it's a constant, as it better be later.  */

	  offset = build_int_2 (TREE_INT_CST_LOW(offset), 
				TREE_INT_CST_HIGH(offset));
	  TREE_TYPE (offset) = TREE_TYPE(signed_size_zero_node);
	  this_offset = size_binop (MINUS_EXPR, offset, base_offset);
  ...
  while (virtuals)
      ...
      else
	{
	    ...
	    }
	  {
	    ...
	    TREE_VALUE (virtuals) = build_vtable_entry (signed_size_zero_node, vfn);
	  }
	}
      virtuals = TREE_CHAIN (virtuals);
     ...
     int warn_anon;
{
  ...
  if (pending_virtuals)
      ...
      if (first_vfn_base_index < 0)
	{
	  ...
	  set_rtti_entry (pending_virtuals, signed_size_zero_node, t);
