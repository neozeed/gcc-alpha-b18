...
#if defined(USG) && !defined(MIPS) && !defined (hpux) && !defined(_WIN32) && !defined(__linux__) && !defined(__OPENNT) && !defined(CROSS_COMPILE)
#include <syms.h>
...
void
sdbout_symbol (decl, local, fnname)
     tree decl;
     int local;
     char *fnname;
{
...
/* Output sdb info for the current function name.
   ...
   Since the name used by assemble_start_function might not be the one
   we get out of current_function_decl (NT/Windows stdcall, e.g.), use
   the provided name */

void
sdbout_mark_begin_function (fnname)
char *fnname;
{
  sdbout_symbol (current_function_decl, 0, fnname);
}
...
void
sdbout_minimal_mark_begin_function (fnname)
char *fnname;
{
  PUT_SDB_DEF (fnname);
     ...
     int line;
{
  ...
  MAKE_LINE_SAFE (line);
  /* Warning, Will Robinson... the MS compilers don't subtract out the
     beginning line for .ef entries.  Rather than fix it here, we bandaid
     it in gas, so the assembly language remains standard */
