...
/* When casting a pointer to an integer, we can't assume that anything
   including HOST_LONG_INT is always safe.  (Alpha/NT with 64 bit 
   HOST_LONG_INT and 32 bit pointers!) */
#ifndef HOST_PTR_AS_INT
#define HOST_PTR_AS_INT HOST_WIDE_INT
#endif
typedef HOST_PTR_AS_INT host_ptr_as_int;
...
/* Get the number of registers needed to represent this mode (for
   "real" modes only (not VOIDmode/BLKmode).  */

extern short mode_registers_needed[];
#define GET_MODE_REGISTER_SIZE(MODE)	(mode_registers_needed[(short) (MODE)])
