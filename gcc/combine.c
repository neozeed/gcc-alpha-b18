...
/* Record a bunch of changes to be undone.
   ...
   last_insn shadows the corresponding global, and old_last_next is the
   value it had.  This is because some of the calls below can end up
   calling add_insn (and that may be a bad thing in itself), and add
   insns after the end-procedure.  (For the skeptics out there, one
   such sequence is: 
   simplify_rtx -> simplify_unary_operation -> convert_memory_address ->
   convert_modes -> convert_move -> emit_unop_insn -> copy_to_mode_reg ->
   emit_move_insn -> emit_move_insn_1 -> emit_insn -> add_insn )
   The loop in combine_instructions will see such insns, and try to operate
   on them; they might contain new registers, causing the alloca()'d arrays
   to be overrun.  */
...
static void init_undos		PROTO((void));
...
init_undos()
{
  ...
  undobuf.other_insn = 0;
  undobuf.last_insn = get_last_insn();
  undobuf.old_last_next = NEXT_INSN(undobuf.last_insn);
}
...
undo_all ()
{
  ...
  undobuf.undos = undobuf.previous_undos = 0;
  NEXT_INSN(undobuf.last_insn) = undobuf.old_last_next;
  set_last_insn(undobuf.last_insn);
