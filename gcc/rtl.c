...
/* Indexed by machine mode, gives the size, in registers, of the
   object when loaded into a register.  Used by calls/functions
   when handling register args/results. Alto this could be calculated
   at runtime, the divides make it very expensive to do so when we
   can do it here once. */

#define DEF_MACHMODE(SYM, NAME, CLASS, SIZE, UNIT, WIDER)  \
 (SIZE==0?0:                                                             \
  (((MAX(UNIT,UNITS_PER_WORD) + (UNITS_PER_WORD - 1)) / UNITS_PER_WORD) \
     * (SIZE/UNIT))) ,

short mode_registers_needed[(int) MAX_MACHINE_MODE] = {
#include "machmode.def"		/* machine modes are documented here */
};
