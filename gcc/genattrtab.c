...
#if !defined(VMS) && !defined(USG) && !defined(__OPENNT)
#include <sys/time.h>
...
/* Here is how primitive or already-shared RTL's hash
   codes are made.  */
#define RTL_HASH(RTL) ((host_ptr_as_int) (RTL) & 0777777)
...
attr_rtx VPROTO((enum rtx_code code, ...))
{
  ...
  if (GET_RTX_CLASS (code) == '1')
      ...
      if (! RTX_INTEGRATED_P (arg0))
	{
      ...
      hashcode = ((host_ptr_as_int) code + RTL_HASH (arg0));
      ...
      if (! RTX_INTEGRATED_P (arg0) || ! RTX_INTEGRATED_P (arg1))
	{
      ...
      hashcode = ((host_ptr_as_int) code + RTL_HASH (arg0) + RTL_HASH (arg1));
      for (h = attr_hash_table[hashcode % RTL_HASH_SIZE]; h; h = h->next)
	if (h->hashcode == hashcode
	    && GET_CODE (h->u.rtl) == code
	    && XEXP (h->u.rtl, 0) == arg0
...
/* CYGNUS LOCAL haifa */
#ifdef HAIFA
	      blockage = op->issue_exp;
#else
/* END CYGNUS LOCAL */
	      blockage = operate_exp (POS_MINUS_OP, readycost,
					  make_numeric_value (1));
