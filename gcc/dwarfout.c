     ...
     register void *arg;
{
  ...
  sibling_attribute ();
  equate_type_number_to_die_number (type); /* CYGNUS LOCAL fnf, cvs rev 1.23 */
  member_attribute (TYPE_CONTEXT (type));
  if (1) /* CYGNUS LOCAL fnf, cvs rev 1.23 */
    {
    ...
    {
      /* a varying length string, needs proper location (FIXME) */
