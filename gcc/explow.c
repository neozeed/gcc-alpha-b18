...
extern void do_jump_for_compare	PROTO((rtx, rtx, rtx));
     ...
     rtx x;
{
  ...
  switch (GET_CODE (x))
    ...
    case LABEL_REF:
      ...
      LABEL_OUTSIDE_LOOP_P (temp) =  LABEL_OUTSIDE_LOOP_P (x);
      return temp;
...
allocate_dynamic_stack_space (size, target, known_align)
     ...
     int known_align;
{
  rtx bypass_label = NULL;

  /* If we're asking for zero bytes, it doesn't matter what we point
     to since we can't dereference it.  Return NULL to match
     malloc() and the Gwyn alloca().  (See also below.) */
  if (size == const0_rtx)
    return const0_rtx;
  ...
  /* Havoc will ensue if size is zero because if we probe it with
     a write, we might hit something important.  (Specifically,
     the return address on the alpha.)  It would be nice to
     somehow range check size as well, but what do we compare
     with? Or is failing a stack allocate an OK answer?  */

  {
      rtx comp = compare_from_rtx (size, const0_rtx,
				   EQ, 1, Pmode, NULL_RTX, 0);
      if (comp == const_true_rtx)
        return const0_rtx;
      else if (comp != const0_rtx)
	{
	  bypass_label = gen_label_rtx ();
	  /* unconditionally zero target; it's better than a branch */
	  emit_move_insn (target, const0_rtx);
	  do_jump_for_compare (comp, NULL_RTX, bypass_label);
	}
    }
     
  /* If we get here, show we're calling alloca or equivalent.  */
...
#define MUST_ALIGN (STACK_BOUNDARY < BIGGEST_ALIGNMENT)
  ...
  /* here, and below, if we're on a machine with 32-bit pointers, and
     64-bit registers, we have to convert (at least sometimes) */
  convert_move (target, virtual_stack_dynamic_rtx,1);
#endif
