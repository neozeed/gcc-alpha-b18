     ...
     HOST_WIDE_INT c;
{
  ...
  /* Treat an access through an AND (e.g. a subword access on an Alpha)
     ...
     This is trickier than it seems; the easy thing to do is to 
     simply say "it conflicts" and give up.  The best that can be
     done is to separate by general storage class (stack, gp, global)
     unless the addresses are known (enough); in the case of externs
     that's simply not possible.

     The underlying problem is that the sub-quadword access this code
     represents may be storing into the memory for a variable other than
     the one it's actually targeting (load, shift-and-mask, store, leaving
     the adjacent, not-targeted variable unchanged (but rewritten)), and
     if the other variable in that same quadword is also being modified
     nearby, and the load needed for the shift-and-mask occurs before the
     store of the other variable, we lose.

     To do even that requires that the XEXP(?,0) below be converted to
     get the memory address corresponding to the register which in the
     face of register copies (particularly for 32-bit pointers on NT gets),
     and then the test be w.r.t. the general storage class, and if in the
     same class, and the address is known, determine if they are adjacent.
     ...
     int (*varies)();
{
  ...
  /* needed for Alpha; base_alias_check can skip this test...
     (see memrefs_conflicts_p for details) */
  if (GET_CODE (x_addr) == AND || GET_CODE (mem_addr) == AND) return 1;
  ...
  /* below for Alpha... just in case after canonocalization */
  if (GET_CODE (x_addr) == AND || GET_CODE (mem_addr) == AND) return 1;
  ...
  if (MEM_IN_STRUCT_P (x) == MEM_IN_STRUCT_P (mem)
      ...
      /* || GET_CODE (x_addr) == AND || GET_CODE (mem_addr) == AND done above */
     ...
     rtx x;
{
  ...
  /* below for Alpha... just in case after canonocalization */
  if (GET_CODE (x) == AND || GET_CODE (mem) == AND) return 1;
  ...
  return (memrefs_conflict_p (SIZE_FOR_MODE (mem), mem_addr,
	  ...
	  && ! (MEM_IN_STRUCT_P (mem) && rtx_addr_varies_p (mem)
		&& GET_MODE (mem) != QImode
		/* && GET_CODE (XEXP (mem, 0)) != AND now redundant */
		&& ! MEM_IN_STRUCT_P (x) && ! rtx_addr_varies_p (x))
	  && ! (MEM_IN_STRUCT_P (x) && rtx_addr_varies_p (x)
		&& GET_MODE (x) != QImode
		/* && GET_CODE (XEXP (x, 0)) != AND now redundant */
		&& ! MEM_IN_STRUCT_P (mem) && ! rtx_addr_varies_p (mem)));
}
