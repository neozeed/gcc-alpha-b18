...
void uni2ansi(LPWSTR wcs, char *mbs)
{
    if(wcs)
...
int enum_users(LPWSTR servername)
{
    USER_INFO_3 *buffer;
    DWORD entriesread = 0;
    ...
    {
        DWORD i;
        DWORD rc = NetUserEnum(servername, 3, FILTER_NORMAL_ACCOUNT, (LPBYTE *) &buffer,
                     1024,
        ...
        for(i=0; i<entriesread; i++)
        {
            ...
            printf("%s::%d:%d:%s:%s:/bin/sh\n", username, uid, gid, fullname, homedir_psx);
        }
...
int enum_local_groups()
{
    ...
    do
    {
        ...
        for(i=0; i<entriesread; i++)
        {
            ...
            if(!LookupAccountName(NULL, localgroup_name, psid, &sid_length, domain_name, &domname_len,
                              &acc_type))
            {
