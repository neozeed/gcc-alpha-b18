...
#define WIN95_EVENT_LOG_PATH "C:\\CYGWIN32_SYSLOG.TXT"
#define CYGWIN32_LOG_NAME "CygWin32"
 ...
 * We could, however, add the resources in cygwin32 and
...
syslog (int priority, const char *message,...)
{
    ...
    *dst = '\0';
	...
	if (process_logopt & LOG_PID)
	  {
	    if (pass.print ("Win32 Process Id = 0x%X : CygWin32 Process Id = 0x%X : ",
	...
	HANDLE hEventSrc = RegisterEventSourceA (NULL, (process_ident != 0) ?
					 process_ident : CYGWIN32_LOG_NAME);
	if (hEventSrc == 0)
	  {
	    debug_printf ("RegisterEventSourceA failed with %x\n",
			  GetLastError ());
	    return;
	  }
	...
	*/
	HANDLE fHandle = myself->hmap[fileno (fp)].h->get_handle ();
	if (LockFile (fHandle, 0, 0, 1, 0) == FALSE)
	  {
	    debug_printf ("failed to lock file %s\n", get_win95_event_log_path());
	    fclose (fp);
	    return;
	  }
