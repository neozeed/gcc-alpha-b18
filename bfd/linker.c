...
enum link_row
{
  ...
  SCTN_ROW		/* Section Symbol ("addative common").  */
};
...
enum link_action
{
  ...
  SCTN,         /* Add a section symbol */
  SDEF,         /* warn about references to section symbol */
  SCREF         /* warn about common references to section symbol */
};

/* The state table itself.  The first index is a link_row and the
   second index is a bfd_link_hash_type.  */

static const enum link_action link_action[9][9] =
{
/* curr\prev  new     undef  undefw def    defw   com    indr   warn   sctn */
/* UNDEF  */  {UND,   NOACT, UND,   REF,   REF,   NOACT, REFC,  WARNC, REF   },
/* UNDEFW */  {WEAK,  NOACT, NOACT, REF,   REF,   NOACT, REFC,  WARNC, REF   },
/* DEF 	  */  {DEF,   DEF,   DEF,   MDEF,  DEF,   CDEF,  MDEF,  CYCLE, SDEF  },
/* DEFW	  */  {DEFW,  DEFW,  DEFW,  NOACT, NOACT, NOACT, NOACT, CYCLE, WARN  },
/* COMM	  */  {COM,   COM,   COM,   CREF,  CREF,  BIG,   CREF,  WARNC, SCREF },
/* INDR	  */  {IND,   IND,   IND,   MDEF,  IND,   CIND,  MIND,  CYCLE, WARNC },
/* WARN   */  {MWARN, WARN,  WARN,  CWARN, CWARN, WARN,  CWARN, MWARN, WARN  },
/* SET	  */  {SET,   SET,   SET,   SET,   SET,   SET,   CYCLE, CYCLE, WARNC },
/* SCTN	  */  {SCTN,  SCTN,  SCTN,  SDEF,  SDEF,  SCREF, WARN,  WARNC, NOACT },
};
     ...
     struct bfd_link_hash_entry *h;
{
  ...
  switch (h->type)
    ...
    /* bfd_link_hash_defined_ext and bfd_link_hash_defweak_ext entries 
       do not need to be reflected here */
    }
  /*NOTREACHED*/
}
     ...
     struct bfd_link_hash_entry **hashp;
{
  ...
  if (bfd_is_ind_section (section)
    ...
    row = COMMON_ROW;
  else if ((flags & BSF_SECTION_SYM) != 0)
    row = SCTN_ROW;
  else
    row = DEF_ROW;
	...
	case DEFW:
	  {
	    ...
	    /* If we have been asked to, we act like collect2 and
	       ...
	       automatically.  */
	    if (collect && name[0] == '_' && h->type != bfd_link_hash_section)
	      {
		const char *s;
...
#define CONS_PREFIX_LEN (sizeof CONS_PREFIX - 1)
	...
	case SCREF:
	  /* a section symbol trying to be treated as common */
	case CREF:
	  {
	    bfd *obfd;
	  ...
	  || bfd_is_ind_section (bfd_get_section (sym)))
	{
	  ...
	  if (h != (struct generic_link_hash_entry *) NULL)
		...
		case bfd_link_hash_defined_ext:
		  sym->flags |= BSF_GLOBAL;
		  sym->flags &=~ BSF_CONSTRUCTOR;
		  sym->value = h->root.u.defext.perm->value;
		  sym->section = h->root.u.defext.perm->section;
		  break;
		case bfd_link_hash_defweak:
		  sym->flags |= BSF_WEAK;
		  sym->flags &=~ BSF_CONSTRUCTOR;
     ...
     struct bfd_link_hash_entry *h;
{
  ...
  switch (h->type)
    ...
    case bfd_link_hash_defweak_ext:
      ...
      /* drop thru */
    case bfd_link_hash_defined_ext:
      sym->section = h->u.defext.perm->section;
      sym->value = h->u.defext.perm->value;
      break;
    case bfd_link_hash_common:
