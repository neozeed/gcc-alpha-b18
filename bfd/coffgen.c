     ...
     unsigned int target_index;
{
  ...
  /* styp_to_sec_flags needs target_index */
  return_section->target_index = target_index;
  /* return section may be modified */
  return_section->flags = bfd_coff_styp_to_sec_flags (abfd, hdr, name, 
      return_section);
...
{
  flagword oflags = abfd->file_flags;
  ...
  if (!(internal_f->f_flags & F_RELFLG))
    abfd->file_flags |= HAS_RELOC;
  if ((internal_f->f_flags & F_EXEC))
    abfd->file_flags |= EXEC_P;
  if (!(internal_f->f_flags & F_LNNO))
    abfd->file_flags |= HAS_LINENO;
  if (!(internal_f->f_flags & F_LSYMS))
    abfd->file_flags |= HAS_LOCALS;

  /* FIXME: How can we set D_PAGED correctly?  */
  if ((internal_f->f_flags & F_EXEC) != 0)
    abfd->file_flags |= D_PAGED;

  bfd_get_symcount (abfd) = internal_f->f_nsyms;
  if (internal_f->f_nsyms)
    abfd->file_flags |= HAS_SYMS;
     ...
     struct internal_syment *syment;
{
  ...
  /* Should absolute section be addressed here? */
  else
    {
     ...
     bfd *bfd_ptr;
{
  ...
  for (symbol_index = 0; symbol_index < symbol_count; symbol_index++)
	...
	{
	  int i;
	  combined_entry_type *s = &coff_symbol_ptr->native[0];
	  ...
	  for (i = 0; i < s->u.syment.n_numaux; i++)
	    {
	      combined_entry_type *a = &coff_symbol_ptr->native[i+1];
	      if (a->fix_tag)
		{
     ...
     bfd_size_type *debug_string_size_p;
{
  ...
  if (native->u.syment.n_sclass == C_FILE
	...
	{
	  if (name_length <= bfd_coff_auxesz (abfd))
	    {
	      strncpy (auxent->x_file.x_fname, name, bfd_coff_auxesz(abfd));
	    }
	  else
	    {
     ...
     bfd_size_type *debug_string_size_p;
{
  ...
  if (bfd_is_und_section (symbol->section))
	...
	if (c != (coff_symbol_type *) NULL)
	  native->u.syment.n_flags = bfd_asymbol_bfd (&c->symbol)->file_flags;
      }
    }
...
#if 0 // this would be required if we support WEAK
  else if (symbol->flags & BSF_WEAK)
    native->u.syment.n_sclass = C_NT_WEAK;
#endif
  else
	  ...
	  && internal_ptr->u.syment.n_numaux > 0)
	{
	  ...
	  if ((internal_ptr + 1)->u.auxent.x_file.x_n.x_zeroes == 0)
	  ...
	  else
	    {
	      ...
	      /* in MS Coff, the name may not be in the string table, but
		 rather form multiple AUX entries.  Look at naux; we
		 can't cheat because the internal entries aren't the same
		 size as external */
		char *tem = bfd_alloc(abfd, bfd_coff_auxesz (abfd)
			              *internal_ptr->u.syment.n_numaux + 1);
		/* it's probably not worth the trouble to trim down the name
		   to the minimum; big by an average of 9 bytes */
		int i;

		for (i=0; i<internal_ptr->u.syment.n_numaux; i++) 
		  {
		     strncpy(&tem[bfd_coff_auxesz (abfd) * i], 
		       (internal_ptr + 1 + i)->u.auxent.x_file.x_fname,
		       bfd_coff_auxesz (abfd));
		  }
	        internal_ptr->u.syment._n._n_n._n_offset = (long) tem;
	    }
	}
      else
	{
  ...
  if (section->lineno != NULL)
      ...
      for (; i < section->lineno_count; i++)
	{
	  if (l->line_number == 0)
		...
		break;
	      *line_ptr = l->line_number + line_base 
		        - ((coff_data(abfd)->pe)?0:1);
	    }
	  l++;
	}
    }
