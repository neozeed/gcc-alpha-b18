...
#ifdef IMAGE_REL_ALPHA_PAIR
  if (reloc_dst->r_type == IMAGE_REL_ALPHA_PAIR)
    {
      /* the MS compiler doesn't sign extend this 16 bit field, so we
	 do it here (it won't hurt if it already is, because it's only 16
	 bits) */
      reloc_dst->r_symndx = (reloc_dst->r_symndx<<16) >> 16;
    }
#endif
     ...
     PTR	     dst;
{
  ...
  /* There are really two magic numbers involved; the magic number that
     says this is a NT executable (PEI) and the magic number that determines
     the architecture.  If the NT magic number isn't valid, the architecture
     magic number could be mimiced by some other field (specifically,
     the number of relocs in section 3).  Since we know we're dealing with
     PEI here, we'll munge the magic number if it really isn't us. */

  if ( *(short *) &(filehdr_src->e_magic) != DOSMAGIC)
...
static void coff_swap_sym_in (bfd*, PTR, PTR);
...
#define SA(x)  (((x) + sa -1 ) & (- sa))
  ...
  {
    /* MSVC4.0 for Alpha creates .bss with contents and sets 0 size 
       in this field; we reflect that based on input bss flags. */
    /* Properly, the input value to here should not be arrived at by
       name, (but by checking the flags).  However, since it is,
       we have to do it by name, too */
    asection *sec = bfd_get_section_by_name (abfd, ".bss");
    if (sec && (sec->flags && SEC_ALLOC)!=0 && (sec->flags && SEC_LOAD)==0)
       aouthdr_in->bsize = 0;
  }
  ...
  PTR	     in;
{
  ...
  /* MS handles overflow of line numbers by carrying into the reloc field
     (it appears).  Since it's supposed to be zero for PE *IMAGE* format,
     that's safe.  This is still a bit iffy. */
#ifdef COFF_IMAGE_WITH_PE
  ...
  /* If this is uninitialized data, use the virtual size (stored in
     s_paddr!!) instead of the physical size */
     ...
     PTR	out;
{
  ...
  /* NT wants the size data to be rounded up to the next NT_FILE_ALIGNMENT,
     but zero if it has no content (as in .bss, sometimes) */
     ...
     PTR aouthdr;
{
  ...
  pe->coff.datestamp = internal_f->f_timdat;
