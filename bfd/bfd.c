...
/* Shamelessy stolen from the assembler */
/* _bfd_abort: Print a friendly message saying how totally hosed we are,
...
_bfd_abort (file, line, fn)
     ...
     int line;
{
  fprintf (stderr, "BFD Internal error, aborting at %s line %d", file, line);
  if (fn)
     ...
     bfd *abfd;
{
  ...
  if ((abfd->file_flags & BFD_IN_MEMORY) != 0)
