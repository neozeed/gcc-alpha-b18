...
The span of the "ordinary" jump instruction (in bytes).  (Ordinary==
the instruction the compiler generates for calls.)  0 -> no limit.
(Limited length branches are presumed to be (signed) PC relative,
so this is the absolute value of the distance that the jump can reach.)

.  bfd_vma jump_span;

The number of bytes a branch stub takes up.  Ignored if above is zero.

.  short stub_length;

The desired initial alignment for a stub section

.  short stub_align; 

The page size; stub branch logic tries to cluster stub branches into
convienient page chunks.  If zero, no clustering.
...
extern const bfd_target nt_alphape_vec;
extern const bfd_target nt_alphapei_vec;
