...
#define DOIT(x) \
  x = ((x & ~howto->dst_mask) | (((x & howto->src_mask) + diff) & howto->dst_mask))
	...
	/* since we apply addend here, tell everyone else we did so! */
	reloc_entry->addend -= diff;
...
#ifdef COFF_WITH_PE
  *addendp = 0;  /* cancel out code in _bfd_coff_generic_relocate_section */
#endif
...
static const bfd_target * i3coff_object_p PARAMS((bfd *a));
...
#endif
{
  ...
  0,				/* Jump span (0 == no limit) */
  0,				/* size of stub branch */
  0,				/* align of stub branch */
  0,				/* page size for stub branch */
