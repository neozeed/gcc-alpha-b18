...
static char *coff_getfilename PARAMS ((union internal_auxent *, int));
...

static void
record_minimal_symbol (name, address, type, objfile, bfd_section)
     char *name;
     CORE_ADDR address;
  ...
  if (info->stabsects)
    {
	 ...
	 bfd_get_section_contents?  */
      ...
      struct section_offsets *new_so;
#ifdef COFF_IMAGE_WITH_PE
	  long int Imagebase;
      /* section_offsets is for COFF symbols (which don't contain
	 ImageBase).  For stabs symbols, we need to back out ImageBase. */
      new_so = (struct section_offsets *)
         obstack_alloc (&objfile -> psymbol_obstack, SIZEOF_SECTION_OFFSETS);
      memcpy(new_so, section_offsets, 
	 sizeof(struct section_offsets) * SECT_OFF_MAX);

      for (i = 0; i < SECT_OFF_MAX; i++) {
#if 1
		  Imagebase= bfd_getImageBase(abfd);
#endif
		  ANOFFSET (new_so, i) -= bfd_getImageBase(abfd); 
	  }
#else
      new_so = section_offsets;
#endif
     ...
     struct objfile *objfile;
{
	...
	{
	  s->addr += bfd_getImageBase(objfile->obfd);
	  s->endaddr += bfd_getImageBase(objfile->obfd);
	}
}
     ...
     struct objfile *objfile;
{
  ...
  first_function_last_file = ANOFFSET (section_offsets, SECT_OFF_TEXT);
      ...
      switch (cs->c_sclass)
	{
	  ...
	  case C_LABEL:
		...
		break;
		 ...
		 that look like this.  Ignore them.  */
	      break;
	    /* Alpha local labels look like $nnn or $Cnnn */
	    if (cs->c_name[0]=='$') 
		break;
	    /* On Intel, they also can be Lnnn; we can discard them
	  ...
	  case C_EXT:
	      ...
	      else
		{
		  ...
		  switch (sec)
		    ...
		    case SECT_OFF_RODATA:
		      ms_type =
			external_class ? mst_text : mst_file_text;
		      break;
		    case SECT_OFF_DATA:
		      ms_type =
			external_class ? mst_data : mst_file_data;
		      break;
		    case SECT_OFF_BSS:
		      ms_type =
			external_class ? mst_bss : mst_file_bss;
		      break;
		    default:
		      ms_type = mst_unknown;
		      break;
		...
		prim_record_minimal_symbol_and_info
		  (cs->c_name, tmpaddr, ms_type, NULL, sec, 
	          coff_section_from_bfd_index(objfile->obfd, cs->c_secnum),
		  objfile);
    ...
    register union internal_auxent *aux;
{
  ...
  if (cs->c_naux >= 1)
    ...
    for (i = 1; i < cs->c_naux; i++)
      bfd_read (&temp_aux[i*local_auxesz], local_auxesz, 1, nlist_bfd_global);
    }
...
static char *
coff_getfilename (aux_entry, naux)
    ...
    int naux;
{
  ...
  /* FIXME: We should not be throwing away the information about what
     ...
     place.  */
  if ((temp = max(strrchr (result, '/'), strrchr(result, '\\'))) != NULL)
    result = temp + 1;
  return (result);
}

     ...
     CORE_ADDR addr;
{
  ...
  for (i = 0; i < SECT_OFF_MAX; i++)
    ANOFFSET (section_offsets, i) = addr + bfd_getImageBase(objfile->obfd);
     ...
     PTR table_pp_char;
{
  ...
  (*table_pp)->addr = bfd_section_vma (abfd, asect) + bfd_getImageBase(abfd);
...
static struct sym_fns coff_sym_fns =
{
  ...
  coff_symfile_offsets,
			/* sym_offsets:  xlate external to internal form */
  coff_build_section_table,
			/* build_section_table: exec time section offsets */
