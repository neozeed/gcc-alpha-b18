  ...
  if (args)
	...
	error ("no exec file name was specified");
      ...
      scratch_chan = openp (getenv ("PATH"), 1, filename, 
			    ...
			    &scratch_pathname);
#if (defined(__GO32__) || defined(_WIN32)) && !defined(__OPENNT)
      if (scratch_chan < 0)
      {
...
static void
default_add_to_section_table (abfd, asect, table_pp_char)
     bfd *abfd;
     sec_ptr asect;
     PTR table_pp_char;
{
...
int
default_build_section_table (some_bfd, start, end)
     bfd *some_bfd;
  ...
  if (abfd == exec_bfd)
    {
      ...
      print_address_numeric (bfd_get_start_address (abfd) + bfd_getImageBase(abfd), 1, gdb_stdout);
      printf_filtered ("\n");
    }
