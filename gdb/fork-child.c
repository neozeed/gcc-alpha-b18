     ...
     char *shell_file;
{
...
#ifdef __OPENNT
  if (shell_file == NULL && getenv("OPENNT_ROOT") != NULL)
    shell_file = concat(getenv("OPENNT_ROOT"), "/", default_shell_file, NULL);
#endif
  if (shell_file == NULL)
  ...
  if (pid == 0)
	...
	sleep (debug_fork);
      ...
      /* Now that we're in the child, we need to close off any of our
	 open fds so the target inferior gets a clean slate; this might be
	 a vfork(), so we don't want to call the "official" close routines,
	 which change the state of memory; rather we just want the fd's closed.

	 Special note/FIXME: if/when close_exec_file actually does anything,
	 this may need to not do exec_bfd.
      */

      if (exec_bfd) close(bfd_fileno(exec_bfd));
      if (core_bfd) close(bfd_fileno(core_bfd));
      close_all_objfile_fds();
