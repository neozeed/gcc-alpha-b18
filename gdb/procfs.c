...
/*  PROCFS_USE_READ_WRITE really means two things:
    - use read()/write() instead of ioctl()
    - certain ioctl() "read" operations are replaced by fields
      in prstatus; reading status (or doing nothing) replaces
      the ioctl().  (In some instances, this also changes where
...
struct greg_ctl {
        int             cmd;
	/* On a 64-bit architecture, the struct will be 64-bit aligned,
	   but int may not be 64 bits, so just treat it as space */
        char	        gregset[sizeof(gregset_t)];
};
...
struct fpreg_ctl {
	...
	/* On a 64-bit architecture, the struct will be 64-bit aligned,
	   but int may not be 64 bits, so just treat it as space */
        char	        fpregset[sizeof(fpregset_t)];
};
...
/* We need to leak out the complete status (for printing floating point
   control registers, if present); it might be possible to put the control
   registers where they can be accessed thru the normal gdb mechanisms,
   but if all we want to do is print them, it's rather messy.  Other
   instantiations (using ptrace) make direct ptrace calls in this situation */
...
#else /* LOSING_POLL */
    (void) procfs_read_status(pi);  /* we need the side-effects */
#endif /* LOSING_POLL */
     ...
     struct target_ops *target; /* ignored */
{
  ...
  if (lseek(pi->as_fd, (off_t) memaddr, SEEK_SET) == (off_t) memaddr)
    ...
    {
      error("lseek of /dev/proc failed");
    }
  return (nbytes);
}
     ...
     int regno;
{
  ...
  fill_gregset (&pi->prstatus.pr_lwp.pr_context.uc_mcontext.gregs, regno);
  ...
  fill_fpregset (&pi->prstatus.pr_lwp.pr_context.uc_mcontext.fpregs, regno);
...
#endif
#endif /* HAVE_NO_PRRUN_T */

  if (!procfs_read_status (pi))
    proc_init_failed (pi, "procfs_read_status failed");

#ifdef __OPENNT
  /* On OpenNT, the location of the trampoline code can float, and it's
     not part of the symbol table (because it's internal to a DLL).  However,
     the system knows where it is, and tells us, so we capture that
     info here. */
...
#ifndef HAVE_STRUCT_LWP   /* overload... don't need the test at all since
                                         it's a Solaris bug */
     ...
     int pid;
{
  ...
  procfs_set_syscall_trap (pip, SYS_execve, PROCFS_SYSCALL_EXIT,
			   procfs_exec_handler);
#endif
  /* see comments for proc_set_exec_trap */
  procfs_set_syscall_trap (pip, SYS_getgroups, PROCFS_SYSCALL_ENTRY,
			   procfs_exec_handler);
#endif  /* PIOCSSPCACT */
...
/* Turn on inherit-on-fork flag so that all children of the target process
  ...
  startup_inferior (START_INFERIOR_TRAPS_EXPECTED+1);
#else
  /* One trap to synchronize, one to to exec the shell, 
     one to exec the program being debugged.  */
  startup_inferior (2+1);
#endif
...
NOTE
	...
	There is an ugly (is there any other kind?) race condition here: 

	The file descriptor we open on ourself to set the trace flags is
	the only open procfs descriptor until the parent opens one.
	- It really shouldn't be left dangling open (with the fd # discarded)
	  at the exit of this routine.  Specifically, the fd will hang around
	  and we'll have a fd open on ourself, which means there never will
	  be a last close.  (The inferior may not appreciate having it 
	  hanging around, either.)
	- However, the parent may not have run to open its file descriptor
	  on the inferior before we close it.
	- We really do want "run on last close" set in case something goes
	  wrong.  However, if it is set, and the parent hasn't opened yet,
	  the child (us) will "run away".
	- In the case of some vfork's, the parent will NEVER run until we
	  get to our exec.

	There's no obvious solution to the problem when vfork is involved;
	either don't use vfork with /proc, or leave the file descriptor open.
	(A really awful kluge might work: prefix the argument to sh -c 
	with exec n<- to close the fd below; this requires passing the fd
	back up to the caller.)

	If vfork is not involved, we can simply add a "little" system
	call (getgroups) to the set of traced calls, and wait for it to
	return.  When it does we know the parent must have a /proc file
	open on us (because it continued us).  This may require changes
	for those systems using START_INFERIOR_TRAPS_EXPECTED.

 */

static void
proc_set_exec_trap (void)
{
  ...
  praddset (&entryset.sysset, SYS_getgroups);  /* so we block later */
  ...
  (void) getgroups(0,NULL);  /* per above; wait for parent to wake us up */

  /* and turn off trapping on this */
  prdelset (&entryset.sysset, SYS_getgroups);  /* so we block later */
#ifdef PROCFS_USE_READ_WRITE
  entryset.cmd = PCSENTRY;
  if (write (fd, (char *) &entryset, sizeof (struct sys_ctl)) < 0)
#else
...
#ifdef PROCFS_USE_READ_WRITE
    pctl.cmd = PCSET;
    /* There was a PR_FORK bit here, undoing the clear above; DON'T
       put it back until you clear it too.  */
    pctl.data = PR_ASYNC;
...
#if defined(PROCFS_USE_READ_WRITE) && defined(__OPENNT)
CORE_ADDR
proc_allocation_base(CORE_ADDR addr)
{
  ...
  pi = find_procinfo (inferior_pid, 1);
...
#endif /* PROCFS_USE_READ_WRITE && __OPENNT */
     ...
     struct target_waitstatus *ourstatus;
{
  ...
  if (pid != -1)
	...
	break;
#else
  /* if we can't do the above, at least do something, else pi is undefined.
     ??? Is this right */
    wait_again:
     ...
     struct procinfo *pip;
{
  ...
  /* close the control file only if we opened it; otherwise we'd close stdin */
  if (pip->ctl_fd != 0) {
     ...
     int errok;
{
...
#ifdef PROCFS_USE_READ_WRITE
  goterr = procfs_read_status(pi);
  memcpy((char *) &sysctl.sysset, 
	 (char *) &pi->prstatus.pr_sysentry, sizeof (sysset_t));
#else
  goterr = ioctl (pi->ctl_fd, PIOCGENTRY, &sysctl.sysset) < 0;
#endif
  ...
  if (!goterr)
    {
      prdelset (&sysctl.sysset, syscall_num);

#ifdef PROCFS_USE_READ_WRITE
      sysctl.cmd = PCSENTRY;
      if (write (pi->ctl_fd, (char *) &sysctl, sizeof (struct sys_ctl)) < 0
#else
      if ((ioctl (pi->ctl_fd, PIOCSENTRY, &sysctl.sysset) < 0) 
#endif
      && !errok)
	{
	  print_sys_errmsg (pi->pathname, errno);
...
#ifdef PROCFS_USE_READ_WRITE
  /* goterr = procfs_read_status(pi);  Already done above */
  memcpy((char *) &sysctl.sysset, 
	 (char *) &pi->prstatus.pr_sysexit, sizeof (sysset_t));
#else
  goterr = ioctl (pi->ctl_fd, PIOCGEXIT, &sysctl.sysset) < 0;
#endif
  ...
  if (!goterr)
    {
      praddset (&sysctl.sysset, syscall_num);

#ifdef PROCFS_USE_READ_WRITE
      sysctl.cmd = PCSEXIT;
      if (write (pi->ctl_fd, (char *) &sysctl, sizeof (struct sys_ctl)) < 0
#else
      if ((ioctl (pi->ctl_fd, PIOCSEXIT, &sysctl.sysset) < 0)
#endif
      && !errok)
	{
	  procfs_clear_syscall_trap (pi, syscall_num, 1);
