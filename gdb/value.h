...
do { struct type *value_type_arg_tmp = check_typedef (VALUE_TYPE (arg));\
	 ...
	 arg = value_at_lazy (TYPE_TARGET_TYPE (value_type_arg_tmp),	\
			      (CORE_ADDR)unpack_long (VALUE_TYPE (arg), \
					   VALUE_CONTENTS (arg)),       \
...
extern int value_bit_index PARAMS ((struct type *type, char *addr, LONGEST index));
...
extern value_ptr value_slice PARAMS ((value_ptr, LONGEST, LONGEST));
