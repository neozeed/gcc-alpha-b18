...
# TARGET_DEPS are also assumed to be dependancies for EXE

GDB_DEPS=$(MSVCENV) $(EXTRA_DEPS) $(TARGET_DEPS) $(EXE)
!IF "$(STATIC_LINK)" == "0"
GDB_DEPS=$(GDB_DEPS) $(OUTDIR)\serdll32.dll
!ENDIF
!IF "$(DEBUG)" == "_DEBUG"
GDB_DEPS=$(GDB_DEPS) $(BSC)
!ENDIF
GDB_DEPS16= $(MSVCENV16) $(GDB_DEPS16) $(TARGET_DEPS16)
!IF "$(HOST)" == "WIN31"
GDB_DEPS16= $(GDB_DEPS16) $(OUTDIR)\serdll16.dll 
!ENDIF

all_gdb : $(OUTDIR) $(INTDIR) $(SRCDIR) $(MSWINDIR) $(GDB_DEPS) $(GDB_DEPS16) $(RESET)
...
!IF "$(STATIC_LINK)" == "0"
EXTRA_DEPS = $(EXTRA_DEPS) $(OUTDIR)\serdll32.lib
!ENDIF

!IF $(REL) > 963
EXTRA_OBJS = $(EXTRA_OBJS) $(INTDIR)\dwarf2read.obj $(INTDIR)\fnmatch.obj
!ENDIF
!IF "$(DEBUG)" == "_DEBUG"
EXTRA_OBJS = $(EXTRA_OBJS) $(INTDIR)/debugify.obj
!ENDIF
...
!IF "$(STATIC_LINK)" == "1"
COMMON_OBJS= $(COMMON_OBJS) $(INTDIR)/serdll32.obj
!ENDIF
...
$(INTDIR)\$(GDB).lib: $(COMMON_OBJS)
    lib @<<
  /out:$(INTDIR)\$(GDB).lib $(COMMON_OBJS)
<<

#GDB_OBJS=$(TARGET_OBJS) $(INTDIR)\$(GDB).lib
GDB_OBJS=$(COMMON_OBJS) $(TARGET_OBJS) 

#@rem $(DECHO) ======== GDB_OBJS=$(GDB_OBJS) ========
$(EXE) : $(INTDIR) $(OUTDIR) $(DEF_FILE) $(GDB_OBJS) $(EXTRA_DEPS) $(TARGET_DEPS)
    @echo ======== linking $(EXE) =========
...
#!include "serdll.mak"
!IF "$(STATIC_LINK)" == "0"
$(OUTDIR)\serdll32.dll: serdll.mak
	nmake -f serdll.mak $(OUTDIR)\serdll32.dll
$(OUTDIR)\serdll32.lib: serdll.mak
	nmake -f serdll.mak $(OUTDIR)\serdll32.lib
!ENDIF
$(OUTDIR)\serdll16.dll: serdll.mak
	nmake -f serdll.mak $(OUTDIR)\serdll16.dll
$(OUTDIR)\serdll16.lib: serdll.mak
	nmake -f serdll.mak $(OUTDIR)\serdll16.lib
...
RSC_PROJ=/fo$(INTDIR)/"gui.res" $(RSC_FLAGS)
...
!IF "$(STATIC_LINK)" == "1"
$(INTDIR)\serdll32.obj : $(MSWINDIR)\serdll32.c $(MSWINDIR)\serdll32.h
   $(DECHO) $(CPP) $(GDB_CFLAGS) $(CFLAGS) /FR$(INTDIR)/ $(MSWINDIR)\serdll32.c
   $(CPP) @<<
$(GDB_CFLAGS) $(CFLAGS) /FR$(INTDIR)/ $(MSWINDIR)\serdll32.c
<<  
!ENDIF
...
$(INTDIR)\debugify.obj : $(SRCDIR)\gdb\debugify.c $(SRCDIR)\gdb\debugify.h

$(INTDIR)\debugi16.obj : $(SRCDIR)\gdb\debugify.c $(SRCDIR)\gdb\debugify.h
   $(DECHO) $(CPP16) $(GDB_CFLAGS16) $(CFLAGS16)
   copy $(SRCDIR)\gdb\debugify.c $(INTDIR)\debugi16.c
   $(CPP16) @<<
$(GDB_CFLAGS16) $(CFLAGS16) $(INTDIR)\debugi16.c
<<  
...
$(INTDIR)/fnmatch.obj : $(SRCDIR)/libiberty/fnmatch.c
