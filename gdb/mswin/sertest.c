...
#if defined(BUILD_DLL) && !defined(_WIN32s)
int main(void)
#else /* BUILD_DLL */
HANDLE wingdb_hInstance = 0; /* normally defined in wingdb */
...
#if !(defined(BUILD_DLL) && !defined(_WIN32s))
  //HANDLE hInstme = AfxGetInstanceHandle( );
  /* gdb would normally set this for us */
  hInstme = wingdb_hInstance = hInstance; 

  if (!wingdb_hInstance)
  {
    fprintf(stderr, "!wingdb_hInstance\n");
    return 1;
  }
...
/*
  ...
  Return Value
      ...
      If called from within a DLL linked with the USRDLL version of MFC, 
      an HINSTANCE to the DLL is returned.
  Remarks
      This function allows you to retrieve the instance handle of the 
      current application. AfxGetInstanceHandle always returns the 
      HINSTANCE of your executable file (.EXE) unless it is called from 
      within a DLL linked with the USRDLL version of MFC. In this case, 
      it returns an HINSTANCE to the DLL.
*/

  if (!load_serdll16())
  {
    fprintf(stderr, "load_serdll16(x%x)\n", hInstme);
    return 1;
  }
#endif
