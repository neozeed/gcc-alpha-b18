...
$(SRCDIR)\gdb\mswin\prebuilt\code.c : $(SRCDIR)\gdb\mswin\prebuilt\gencode.exe
	@echo ======= building $(SRCDIR)\gdb\mswin\prebuilt\code.c ======
	$(SRCDIR)\gdb\mswin\prebuilt\gencode -x > $(SRCDIR)\gdb\mswin\prebuilt\code.c

$(SRCDIR)\gdb\mswin\prebuilt\gencode.exe: $(SRCDIR)\gdb\gencode.c 
	@echo ======= building $(SRCDIR)\gdb\mswin\prebuilt\gencode.exe ======
	$(CPP) @<<
  $(SRCDIR)\gdb\gencode.c /out=$(SRCDIR)\gdb\mswin\prebuilt\gencode.exe
<<

SIM_OBJS=$(TARGET_SIM_OBJS) $(INTDIR)\sim\interp.obj $(INTDIR)\sim\run.obj \
	$(INTDIR)\sim\callback.obj
SIM_LIBS=$(INTDIR)\libbfd.lib $(INTDIR)\libiberty.lib $(INTDIR)\libbfd.lib \
	$(INTDIR)\libiberty.lib $(INTDIR)\libbfd.lib 
...
!IF $(REL) > 963
BFDREL_OBJS=$(INTDIR)/fnmatch.obj
!ENDIF


LIBBFD_OBJS= $(BFDREL_OBJS)\
	$(TARGET_LIBBFD_OBJS) \
	$(INTDIR)\archive.obj \
	$(INTDIR)\archures.obj \
	$(INTDIR)\bfd.obj \
...
$(OUTDIR)\$(SIM).exe : $(OUTDIR) $(INTDIR) $(INTDIR)/sim/nul $(MSVCENV) $(SIM_OBJS) $(SIM_LIBS) 
