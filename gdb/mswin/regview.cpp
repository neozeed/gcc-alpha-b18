...
#define WIDTHINPIX (m_regsize.cx + ((m_valuesize.cx + gap) * NUM_COLS))
...
void CRegView::OnDraw(CDC* pDC)
{
  ...
  /* New line for PC and current instruction */
  
  pDC->TextOut(cx, cy, "PC ");
  PrintReg(pDoc,pDC, pcreg, 16, m_regsize.cx + gap, cy);
  
  /* And the current insn */
  if (pDoc->m_regsinit[pcreg]) 
    {
      togdb_disassemble (pDoc->m_regs[pcreg], buf);
      cx = m_regsize.cx  + gap + m_valuesize.cx + gap;
...
void CRegView::rethink()
{
  ...
  m_valuesize.cx = len * reg_fontinfo.dunits.cx ;
  m_valuesize.cy = reg_fontinfo.dunits.cy ;

  /* Calculate the maximum register name size.  */
  for (i = 0; reg_order[i] >= 0; i++)
    {
      int rn = reg_order[i];
      int len = strlen (reg_names[rn]);
      if (len > max_regsize)
	max_regsize = len;
    }

  m_regsize.cx = reg_fontinfo.dunits.cx * max_regsize;
