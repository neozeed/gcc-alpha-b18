...
/* I haven't a clue what might trigger this clause, and it seems wrong anyway,
   so I've disabled it until someone complains.  -Stu 10/24/95 */
	...
	/* Order here is important; we don't want to test for a prologue
	   if we're in a trampoline; the prologue may not be valid */
     ...
     enum target_signal oursig;
{
  ...
  printf_filtered ("%*.*s ", 20 - strlen (name), 20 - strlen (name),
		   "                         ");
...
Are you sure you want to change it? ",
		...
		break;
	      default:
	        if (!allsigs && !sigs[signum] &&
		   raw_target_signal_to_host(signum) == 0)
  ...
  if (from_tty)
	...
	{
	  if (sigs[signum] && raw_target_signal_to_host(signum) != 0)
	    {
     ...
     int from_tty;
{
  ...
  if (signum_exp)
      ...
      if (oursig == TARGET_SIGNAL_UNKNOWN)
	{
      ...
      if (raw_target_signal_to_host(oursig) == 0)
