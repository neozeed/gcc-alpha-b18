...
void
alpha_find_saved_regs PARAMS((struct frame_info *frame));

/* Heuristic_proc_start may hunt through the text section for a long
   time across a 2400 baud serial line.  Allows the user to limit this
   search.  */
#ifndef HEURISTIC_FENCE_POST
#define HEURISTIC_FENCE_POST 0
#endif
static unsigned int heuristic_fence_post = HEURISTIC_FENCE_POST;

/* Layout of a stack frame on the alpha:
   -- NOT COMPLETELY ACCURATE FOR NT.
     ...
     CORE_ADDR low_addr;
{
  ...
  memset (link, 0, sizeof (struct linked_proc_info));
  link->next = linked_proc_desc_table;
  linked_proc_desc_table = link;

  proc_desc = &link->info;

#ifndef SIGTRAMP_START_ADDR
#define SIGTRAMP_START_ADDR low_addr
#define SIGTRAMP_END_ADDR low_addr + 3 * 4;
#define SIGTRAMP_FRAME_SIZE 0x298; /* sizeof(struct sigcontext_struct) */
#endif

  proc_desc->numargs = 0;
  PROC_LOW_ADDR (proc_desc)	= SIGTRAMP_START_ADDR;
  PROC_HIGH_ADDR (proc_desc)	= SIGTRAMP_END_ADDR;
  PROC_DUMMY_FRAME (proc_desc)	= 0;
  PROC_FRAME_OFFSET (proc_desc)	= SIGTRAMP_FRAME_SIZE;
...
#if !defined(__OPENNT) && !defined(_WIN32)

     ...
     struct frame_info *frame;
{
  ...
  tmp = wizard_mode? 0 : SKIP_TRAMPOLINE_CODE (pc);
  if (tmp != 0)
    pc = tmp;
...
(more likely) you have encountered a bug in GDB.\n");
    ...
    /* skip (obvious) pads */
    while (read_memory_integer (start_pc, 4) == 0)
       start_pc += 4;
    ...
    struct frame_info *next_frame;
{
    ...
    int pcreg = -1;
    LONGEST registers[32];

    registers[31] = 0;  /* all others are random, anyway */
    ...
    /* 1024 instructions is (at least for NT) the maximum prologue size
       imposed by the Alpha standard */
    /* set a ceiling on limit_pc */
    if (start_pc + 1024*4 < limit_pc)
      limit_pc = start_pc + 1024*4;

    frame_size = 0;

    /* Before we pick out saved registers, we need to know if we have
       a real frame pointer; in the face of alloca() this makes a
       critical difference */

    /* However, we can't overshoot the end of the prolog (when guessing)
       because we might reach a wrong conclusion by looking at another
       function! */
	...
	if (word == 0x6bfa8001)                 /* ret $zero,$(ra),1 */
	  {
	    break;   /* We reached the end of the function; we're done */
	  }
	else if (word == 0x47de040f)			/* bis sp,sp fp */
	  {
	    has_frame_reg = 1;
	    sp = read_next_frame_reg(next_frame, GCC_FP_REGNUM);
	    break;   /* by definition, this instr must by last in the
			prologue, if present at all */
	  }
        /* Look for adjustments of SP; they CANNOT be cumulative by
	   the spec, so the 2'd one means we've got to be done. */
	else if ((word & 0xffff0000) == 0x23de0000)	/* lda $sp,n($sp) */
	  {
	    if (frame_size != 0) 
	      {
		 break;
	...
	else if ((word & 0xffe0001f) == 0x43c0001e)	/* add/sub on sp */
	  {
	    /* Avoids a lot of useless tests, is all */
	    if ((word & 0xffe01fff) == 0x43c0053e)	/* subq $sp,$rx,$sp) */
	      {
		if (frame_size != 0) 
		  {
		     break;
		  }
		frame_size = registers[(word>>16)&0x1f];
	      }
		...
		frame_size = (word>>13) & 0xff;
	      }
	    else if ((word & 0xffe01fff) == 0x43c0141e)	/* addq $sp,n,$sp */
	      {
		/* Exit loop if a positive stack adjustment is found, which
		   usually means that the stack cleanup code in the function
		   epilogue is reached.  */
	    ...
	    else if ((word & 0xffe01fff) == 0x43c0041e)	/* addq $sp,$rx,$sp) */
	      {
		 break;
	      }
	  }
	else if ((word & 0xfc000000) == 0x20000000)	/* lda */
	  {
	     int ra, rb;
	     short disp;
	     ...
	     rb = (word>>16) & 0x1f;
	     disp = word & 0xffff;
	     registers[ra] = registers[rb] + disp;
	  }
	else if ((word & 0xfc000000) == 0x24000000)	/* ldah */
	  {
	     int ra, rb;
	     short disp;
	     ...
	     rb = (word>>16) & 0x1f;
	     disp = word & 0xffff;
	     registers[ra] = registers[rb] + disp*0x10000;
	  }
	else if ((word & 0xffe01fe0) == 0x47e01400)	/* bis $31,n,$rc */
	  {
	     int rc;
	     char lit;
    ...
    /* Now that we know if we have a FP (and can safely use it), pick out
       the saves; this time we know better when to stop, too */
    for (cur_pc = start_pc; cur_pc < limit_pc; cur_pc += 4)
      {
        char buf[4];
	unsigned long word;
...
/* could rewrite prettier by moving up */
    if (has_frame_reg)
...
#if defined(__OPENNT) || defined(_WIN32)
  {
      runtime_function pdata = find_pdata_entry(pc);
      startaddr = 0;
      if (pdata.begin != (unsigned long)-1) 
	{
	   /* We found a pdata entry; we still have to use heuristic...
	      to gather all the info we need, but at least we now have
	      a good begin and prologue end.  The EM data might be
	      in prolog_end, so clean up.

	      This gets the full descriptor information; below, if we find
	      that we're partway thru the prologue, we'll redo it for the
	      partially executed prologue.  */

           proc_desc = heuristic_proc_desc (pdata.begin, 
		 pdata.prolog_end&0xfffffffc, next_frame);
        }
      else 
	  proc_desc = NULL;
  }
...
#ifndef FIND_HEURISTIC_START
#define FIND_HEURISTIC_START (heuristic_fence_post != 0)
#endif
      if (startaddr == 0 || FIND_HEURISTIC_START)
...
#if defined (__OPENNT) || defined(_WIN32)
	  /* Because of the nature of .pdata sections, we'll always have
	     to use heuristic_proc_descs, and we'll want to keep them around.
	     Some things, e.g. the "return" command, need them. */
	  proc_desc=
	    obstack_alloc (&frame_cache_obstack,
				   sizeof (struct alpha_extra_func_info));

	  *proc_desc = temp_proc_desc;
...
#define MASK(i,j) (unsigned long)((((1UL << ((j)+1)) - 1) ^ ((1UL << (i)) - 1))&0xffffffff)
...
skip_trampoline_code (pc, name)
     ...
     char *name;
{
  if (pc && read_memory_unsigned_integer (pc+8, 4) == 0x6bfb0000) /* jmp ($t12) */
    {
      unsigned long i1 = read_memory_unsigned_integer (pc, 4);
      unsigned long i2 = read_memory_unsigned_integer (pc+4, 4);
      CORE_ADDR indirect;
      struct minimal_symbol *indsym;
      char *symname;

      /* At this point we know the third instruction is a jmp; decode the
	 prior two as a ldah/ldl pair, and see where that points */
      indirect = ((i1&0xffff)<<16) + (short)(i2&0xffff);
      indsym = indirect ? lookup_minimal_symbol_by_pc (indirect) : 0;
      symname = indsym ? SYMBOL_NAME(indsym) : 0;
      if (symname) 
	{
...
/* This really ought to be system-defined, but... */
getpagesize()
{
    return 0x2000;
}
...
#if defined (__OPENNT) || defined (_WIN32)
/* support for OpenNT find_saved_regs
   Properly, these next 3 routines belong in alphaopennt-nat.c, but they 
   need so much stuff that's private to this file that it's easier to put here.
   */

extract_one_frame(frame, proc_desc)
     struct frame_info *frame;
     alpha_extra_func_info_t proc_desc;
{
  int ireg;
...
#define SIGFRAME_PC_OFF		offsetof(gregset_t, fir)
#define SIGFRAME_REGSAVE_OFF	offsetof(gregset_t, gregs)
#define SIGFRAME_FPREGSAVE_OFF	offsetof(gregset_t, fpregs)


extract_one_context(frame, sigcontext_addr)
  struct frame_info *frame;
  CORE_ADDR sigcontext_addr;
{
  CORE_ADDR reg_position;
     ...
     struct frame_info *frame;
{
  ...
  if (proc_desc == NULL)
	...
	find a proc_desc, we "synthesize" one using heuristic_proc_desc
	and set the saved_regs right away.  */
     /* This occurs when we get a nonsense pc value (not in any function).
	Since we don't want to abort, just return and pretend it was a
	really minimal function */
     return;

  if (frame->signal_handler_caller)
    {
      CORE_ADDR ra;
      alpha_extra_func_info_t tmp_proc;
      CORE_ADDR context;
      CORE_ADDR start;
#if defined (__OPENNT)
      /* This is messy... (actually AWFUL) the "trampoline" might be 2, 3 
	 or all 5 entities on the frame (sizes below are "current", but the
	 ...
	 Chunk 1:
	    PdxSignalDeliverer's frame = 64
	  + Context struct = 0x230    -- not accounted for in any frame

	 Chunk 2:
	  + PdxNullPosixApi's frame = 176
	  + PdxNullApiCaller's frame = 64
	  + Context struct = 0x230    -- not accounted for in any frame
      ...
      */
      ...
      /* Are we already pointing at PdxNullPosixApi? */
      if (!(frame->pc >= null_start && frame->pc < null_end))
	{
	  /* No... we must be pointing at PdxSignalDeliverer's frame; back up 
	     across the whole mess */

	  /* Extract the frameless context under it */
	  context = read_memory_integer
	     ((frame->next ? frame->next->frame : frame->frame)+0x10,4);
	  extract_one_context(frame, context);
	  frame->frame += sizeof(gregset_t);

	  /* We might be done (no Null API if so) */
          ra = read_memory_integer (frame->saved_regs->regs[PC_REGNUM], 4);
	  ...
	  tmp_proc = find_proc_desc (ra, frame->next);
	  if (tmp_proc == NULL) 
	     error("Unable to unwind thru signal handler (Null Api)\n");
	  extract_one_frame(frame, tmp_proc);

	  /* Account for it in the frame position */
	  frame->frame += PROC_FRAME_OFFSET(tmp_proc);
        }
      else
	{
	  /* We're pointing at the frame FOR PdxNullPosixApi; we need to
	     extract registers for it (because it was skipped in
	     init_extra_frame_info), so back up, and extract. */
	  frame->proc_desc = find_proc_desc (frame->pc, frame->next);

	  frame->frame -= PROC_FRAME_OFFSET(proc_desc);
	  extract_one_frame(frame, proc_desc);
	  frame->frame += PROC_FRAME_OFFSET(proc_desc);
	  proc_desc = frame->proc_desc;
	}

      /* At this point, we're looking at the frame for PdxNullApiCaller,
	 in either case. */
      ra = read_memory_integer (frame->saved_regs->regs[PC_REGNUM], 4);
      ...
      tmp_proc = find_proc_desc (ra, frame->next);
      if (tmp_proc == NULL) 
	 error("Unable to unwind thru signal handler (Null Api Caller)\n");
      extract_one_frame(frame, tmp_proc);
      frame->frame += PROC_FRAME_OFFSET(tmp_proc);

      /* Extract the second context record */
      extract_one_context(frame, frame->frame);
      frame->frame += sizeof(gregset_t);
    }
#else
    error("Unable to unwind thru signal handler (sigtramp)\n");
    }
#endif
  else 
    {
