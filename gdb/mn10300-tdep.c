    ...
    CORE_ADDR pc;
{
  ...
  status = target_read_memory (fi->pc, buf, 2);
  if (status != 0)
    return pc;
  ...
  /* Next we should allocate the local frame.  No more prologue insns
     ...
     Search for add imm8,sp (0xd3XX)
        or	add imm16,a3 (0xf70bXXXX)
        or	add imm32,a3 (0xf467XXXXXX).
     ...
     struct frame_info *fi;
{
  ...
  if (dummy_frame.status & MY_FRAME_IN_FP)
  ...
  else
    {
	 ...
	 at the base of our frame (fi->frame) + <his size>.  */
      return fi->frame + -dummy_frame.stack_size;
    }
}
