/* Tcl/Tk interface routines.
   ...
   Modified by Albert K. Lee <lee@gnat.com>
...
gdbtk_init ()
{
  ...
  if (!gdbtk_lib)
    if (access ("gdbtcl/main.tcl", R_OK) == 0)
      ...
      {
#ifdef __OPENNT
	 /* For OpenNT, we have to prefix the search path; We assume it's
	    a single name in this case */
	 strcpy(gdbtk_lib_tmp, _getInstallPath());
	 strcat(gdbtk_lib_tmp, GDBTK_LIBRARY);
	 gdbtk_lib=gdbtk_lib_tmp;
#else
         gdbtk_lib = GDBTK_LIBRARY;
#endif
      }

  if (gdbtk_lib_tmp != gdbtk_lib) strcpy (gdbtk_lib_tmp, gdbtk_lib);
  found_main = 0;
