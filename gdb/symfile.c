...
#if 0 //ndef IBM6000_TARGET
  /* Apparently it IS a specific hack, and breaks Alpha/NT, too;
     it should (must?) be noved into the sym_init or sym_offsets functions. */
...
#if (defined(__GO32__) || defined(_WIN32)) && !defined(_OPENNT)
  if (desc < 0)
    {
...
simple_read_overlay_table ()
{
  ...
  simple_free_overlay_table ();
  if (msym = lookup_minimal_symbol ("_novlys", 0, 0))
    cache_novlys = read_memory_integer (SYMBOL_VALUE_ADDRESS (msym), 4);
  else 
    return 0;	/* failure */
  if (cache_ovly_table = (void *) 
      xmalloc (cache_novlys * sizeof(*cache_ovly_table)))
    if (msym = lookup_minimal_symbol ("_ovly_table", 0, 0))
      {
...
simple_read_overlay_region_table ()
{
  ...
  simple_free_overlay_region_table ();
  if (msym = lookup_minimal_symbol ("_novly_regions", 0, 0))
    cache_novly_regions = read_memory_integer (SYMBOL_VALUE_ADDRESS (msym), 4);
  else 
    return 0;	/* failure */
  if (cache_ovly_region_table = (void *) xmalloc (cache_novly_regions * 12))
    if (msym = lookup_minimal_symbol ("_ovly_region_table", 0, 0))
      {
